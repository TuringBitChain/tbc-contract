...
const main = async ()=>{
    ...
    const nftContractIds = await contract.API.fetchNFTs(collection_id, address, start, end, network);
    ...
    await contract.API.broadcastTXsraw(txraws.map(txraw => ({ txraw })), network);
}
