...
export type PublishedFTVersion = 1 | 2 | 3 | 4;

export interface PublishedFTCodeClassification {
  readonly version: PublishedFTVersion;
  readonly coin: boolean;
  readonly codeBytes: number;
  readonly partialOffset: number;
}

export interface PublishedFTCodeDescriptor
  extends PublishedFTCodeClassification {
  readonly artifactId: string;
  readonly family: "FT" | "STABLE_COIN";
  readonly tapeSize: number;
  /** Present only for ordinary FT artifacts whose slot-0 lineage has this boundary. */
  readonly originalUTXOWire36Hex?: string;
}

// Backward-compatible classification name retained for callers that only need
// the historical family/version/size fields.
export type PublishedFTArtifact = PublishedFTCodeClassification;

type ByteRange = readonly [start: number, endExclusive: number];

interface FTArtifactDescriptor extends PublishedFTCodeClassification {
  readonly id: string;
  readonly normalizedSha256: string;
  readonly mutableRanges: readonly ByteRange[];
  readonly tapeSizeOffsets: readonly number[];
  readonly originalUTXORange?: ByteRange;
}
...
const ARTIFACTS: readonly FTArtifactDescriptor[] = Object.freeze([
  artifact({
    ...
    partialOffset: 1536,
    normalizedSha256:
      "bfd8126a4cc55725a62b3ae0a7235f6cabe1b15c70f27fe3e7d7fada9bc37c73",
  ...
  artifact({
    ...
    partialOffset: 1536,
    normalizedSha256:
      "aa4471260dc8dbc3e9ba5a7e98653b87e617634b5c37c6025374b73e55310735",
  ...
  artifact({
    ...
    partialOffset: 1856,
    normalizedSha256:
      "f4b034ef624fc3de2dd461996712d4389c31e39fc4f74524e663465f1c7bd4cf",
  ...
  artifact({
    ...
    partialOffset: 1856,
    normalizedSha256:
      "fdfa23a85f44ca26cff96e5053fa33e5b47af7dd3bc31ffbc577ab191952fb82",
  ...
  artifact({
    ...
    partialOffset: 1984,
    normalizedSha256:
      "2c80025bcbe3e42a3fe171547d0f1a576d7bc53a1d8c8fb2ff5c996e56af9172",
  ...
  artifact({
    ...
    partialOffset: 1984,
    normalizedSha256:
      "e08e43f220636197380d8763df8c31c8b4bc76c326f66b9385d4e973d16200b8",
    identityRanges: [
      [258, 278],
      [705, 737],
    ],
  ...
  artifact({
    ...
    partialOffset: 1984,
    normalizedSha256:
      "77b60cc9e884901425987112ff3a9547a0dfd155db641aea0f752ad29b6a30a9",
    identityRanges: [
      [256, 276],
      [669, 701],
    ],
  ...
  artifact({
    ...
    partialOffset: 1984,
    normalizedSha256:
      "0f226cbd6cf89f89d2c22eca43f8295e4bcafb750e11141bc1cee5aebbb42b3b",
    identityRanges: [
      [278, 298],
      [697, 729],
    ],
  ...
  artifact({
    ...
    partialOffset: 2048,
    normalizedSha256:
      "d0a57bdf42b2f919c14febc71aac79ded1f25099cdee8373242261e83150fa74",
  ...
  artifact({
    ...
    partialOffset: 2048,
    normalizedSha256:
      "7d283417e489492719705e2b495775e53f6667751736abe90e2a5d5af14db5d9",
    identityRanges: [
      [302, 322],
      [721, 753],
    ],
 ...
 */
export function classifyPublishedFTCode(
  scriptHex: string,
): PublishedFTCodeClassification | null {
 ...
 * it.
 */
export function decodePublishedFTCode(
  scriptHex: string,
): PublishedFTCodeDescriptor | null {
  const code = decodeStrictHex(scriptHex);
  if (code === null) return null;
  const matched = matchPublishedFTDescriptor(code);
  if (matched === null) return null;
 ...
 */
export function isLegacyFTTape(scriptHex: string): boolean {
  const tape = decodeStrictHex(scriptHex);
  if (tape === null || tape.length < 59) return false;
  ...
  const pushes: Array<{ start: number; dataStart: number; length: number }> = [];
  ...
  if (cursor !== tape.length || pushes.length < 2) return false;
  if (pushes[0].length !== 1) return false;

  const markerPush = pushes[pushes.length - 1];
  return (
    markerPush.start === tape.length - FT_TAPE_MARKER.length &&
    markerPush.length === 5
  );
}

export const isPublishedFTTape = isLegacyFTTape;
...
function artifact(config: {
  ...
  readonly partialOffset: number;
  readonly normalizedSha256: string;
  readonly identityRanges: readonly ByteRange[];
  readonly tapeSizeOffsets: readonly number[];
  readonly originalUTXORange?: ByteRange;
}): FTArtifactDescriptor {
  const controllerRange: ByteRange = [
    config.partialOffset + 1,
    config.partialOffset + 22,
  ];
  const tapeRanges = config.tapeSizeOffsets.map(
    (offset): ByteRange => [offset, offset + 1],
  );
  ...
  return Object.freeze({
    ...
    originalUTXORange: config.originalUTXORange === undefined
      ? undefined
      : Object.freeze([...config.originalUTXORange]) as ByteRange,
  });
}

function matchPublishedFTDescriptor(code: Buffer): FTArtifactDescriptor | null {
  let matched: FTArtifactDescriptor | null = null;
  for (const descriptor of ARTIFACTS) {
    if (code.length !== descriptor.codeBytes) continue;
    if (!hasCanonicalCodeSuffix(code, descriptor.partialOffset)) continue;
    if (!hasConsistentTapeSize(code, descriptor.tapeSizeOffsets)) continue;
...
function decodeStrictHex(value: string): Buffer | null {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length % 2 !== 0 ||
...
function hasCanonicalCodeSuffix(code: Buffer, partialOffset: number): boolean {
  // The partial-hash suffix is exactly:
  // PUSH21 <hash20> <00 address | 01 contract> PUSH5 "2Code".
  if (partialOffset + 28 !== code.length) return false;
  if (code[partialOffset] !== 0x15) return false;
  const controllerOption = code[partialOffset + 21];
  if (controllerOption !== 0x00 && controllerOption !== 0x01) return false;
  return code.subarray(partialOffset + 22).equals(FT_CODE_MARKER);
}

function hasConsistentTapeSize(
  code: Buffer,
  tapeSizeOffsets: readonly number[],
): boolean {
  if (tapeSizeOffsets.length === 0) return false;
  const expected = code[tapeSizeOffsets[0]];
  return tapeSizeOffsets.every(
    (offset) => offset < code.length && code[offset] === expected,
  );
}

function normalizedSha256(
  code: Buffer,
  mutableRanges: readonly ByteRange[],
): string {
...
function readCanonicalPush(
  script: Buffer,
  start: number,
): { start: number; dataStart: number; length: number } | null {
  ...
  if (opcode >= 0x01 && opcode <= 0x4b) {
    ...
    if (length < 0x4c) return null;
  } else if (opcode === 0x4d) {
    if (start + 2 >= script.length) return null;
    dataStart = start + 3;
    length = script.readUInt16LE(start + 1);
    if (length <= 0xff) return null;
  } else if (opcode === 0x4e) {
    if (start + 4 >= script.length) return null;
    dataStart = start + 5;
  ...
  if (dataStart + length > script.length) return null;
