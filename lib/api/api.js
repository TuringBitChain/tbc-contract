...
class API {
  ...
  static async getFTbalance(contractTxid, addressOrHash, network) {
    ...
    const url =
      base_url + `ft/balance/combine/script/${hash}/contract/${contractTxid}`;
    try {
      const response = await (await fetch(url)).json();
  ...
  static async fetchFtPrePreTxData(preTX, preTxVout, network) {
    ...
    for (let i = preTXtape.length - 16; i >= 0; i -= 16) {
      ...
      if (chunk != "0000000000000000") {
        ...
        prepretxdata =
          prepretxdata +
          (0, ftunlock_1.getPrePreTxdata)(
            prepreTX,
            preTX.inputs[inputIndex].outputIndex
          );
      }
    }
      ...
      switch (ftBalanceArray.length) {
        ...
        case 3:
          if (
            ...
            const result = (0, utxoSelect_1.findMinTwoSum)(
              ftBalanceArray,
              amount
            );
        ...
        case 4:
          if (
            ...
            (0, utxoSelect_1.findMinThreeSum)(ftBalanceArray, amount)
          ) {
        ...
        case 5:
          if (
            ...
            if (
              (0, utxoSelect_1.findMinThreeSum)(
                [
        ...
        default:
          if ((0, utxoSelect_1.findMinFiveSum)(ftBalanceArray, amount)) {
            const result_five = (0, utxoSelect_1.findMinFiveSum)(
              ftBalanceArray,
              amount
            );
            if (
              (0, utxoSelect_1.findMinFourSum)(
                [
