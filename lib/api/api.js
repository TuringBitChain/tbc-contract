...
const utxoSelect_1 = require("../util/common/utxoSelect");
const util_1 = require("../util/common/util");
class API {
    static mainnetURL = "https://api.turingbitchain.io/api/tbc/";
    static testnetURL = "https://api.tbcdev.org/api/tbc/";
    /**
    ...
    static async fetchUTXOList(address, network) {
        if (!tbc.Address.isValid(address)) {
            throw new Error("Invalid address input");
        }
    ...
    static async fetchUTXO(privateKey, amount, network) {
        ...
        const amount_bn = Number((0, util_1.parseDecimalToBigInt)(amount, 6));
        try {
    ...
    static async fetchFtUTXOList(contractTxid, addressOrHash, codeScript, network) {
        ...
        const url = base_url + `ft/utxo/combinescript/${hash}/contract/${contractTxid}`;
        try {
            const response = await fetch(url)
                .then((response) => response.text())
                .then((text) => (0, util_1.safeJSONParse)(text));
            const utxoList = response.data.utxos;
            if (utxoList.length === 0) {
    ...
    static async broadcastTXraw(txraw, network) {
        ...
        try {
            ...
            if (data.code === "400" || data.error) {
                const errorMessage = data.data?.error || data.message || "Broadcast failed";
    ...
    static async broadcastTXsraw(txrawList, network) {
        ...
        try {
            ...
            const data = await response.json();
            if (data.code === "200") {
                console.log(`Broadcast success! ${data.data.success} succeeded, ${data.data.failed} failed`);
                return data.data;
            }
            if (data.code === "400" && data.message.includes("partial failure")) {
                console.log(`Partial failure: ${data.data.success} succeeded, ${data.data.failed} failed`);
                return data.data;
            }
            if (data.code === "400") {
    ...
    static async getUTXOs(address, amount_tbc, network) {
        try {
            ...
            const amount_satoshis = Number((0, util_1.parseDecimalToBigInt)(amount_tbc, 6));
            let totalAmount = 0;
    ...
    static async fetchNFTTXO(params) {
        ...
        try {
            ...
            if (tx_hash) {
                ...
                const min_vout_utxo = filteredUTXOs.reduce((prev, current) => prev.index < current.index ? prev : current);
                return {
     ...
     * @throws {Error} Throws an error if the request fails.
     */
    static async fetchNFTs(collection_id, address, start, end, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        const url = base_url +
        ...
        try {
            ...
            if (data.data.nft_list && Array.isArray(data.data.nft_list)) {
                const filteredNFTs = data.data.nft_list.filter((nft) => nft.nft_holder === address);
                return filteredNFTs.map((nft) => nft.nft_contract_id);
            }
            return [];
        }
        catch (error) {
    ...
    static async fetchUMTXO(script_asm, tbc_amount, network) {
        ...
        try {
            ...
            for (let i = 0; i < utxoList.length; i++) {
                if (utxoList[i].value > amount_satoshis &&
    ...
    static async fetchFtUTXOS_multiSig(contractTxid, addressOrHash, codeScript, network) {
        ...
        try {
            ...
            let sortedData = utxoList.sort((a, b) => {
                if (a.ft_value < b.ft_value)
                    return -1;
                if (a.ft_value > b.ft_value)
                    return 1;
                return 0;
    ...
    static async fetchBlockHeaders(network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        const url = base_url + `recentblocks/start/${0}/end/${1}`;
        try {
            const response = await fetch(url);
            if (!response.ok) {
    ...
    static async fetchFrozenTBCBalance(address, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        ...
        try {
            const response = await fetch(url);
            if (!response.ok) {
                throw new Error(`Failed to fetch frozen TBC balance: ${response.statusText}`);
            }
    ...
    static async fetchUnfrozenUTXOList(address, network) {
        ...
        for (const utxo of list) {
            const lockTime = (0, util_1.fetchTBCLockTime)(utxo);
    ...
    static async fetchFrozenUTXOList(address, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        ...
        try {
            const response = await fetch(url);
            if (!response.ok) {
                throw new Error(`Failed to fetch frozen UTXO list: ${response.statusText}`);
            }
            const data = (await response.json()).data.utxos;
            let list = data.map((utxo) => ({
                txId: utxo.txid,
                outputIndex: utxo.index,
                script: "",
                satoshis: utxo.value,
            ...
            const batchSize = 5;
            list = await (0, util_1.fetchInBatches)(list, batchSize, (batch) => Promise.all(batch.map(async (utxo) => {
    ...
    //stableCoin
    static coinInteger(value, field) {
    ...
    static coinNumber(value, field, maximum = Number.MAX_SAFE_INTEGER) {
        const result = API.coinInteger(value, field);
        if (result > BigInt(maximum))
            throw new Error(`Coin API ${field} exceeds ${maximum}`);
        return Number(result);
    }
    static async coinData(url) {
        const response = await fetch(url);
        ...
        try {
            ...
            body = (0, util_1.safeJSONParse)(text.replace(/("supply"\s*:\s*)(\d+)(?=\s*[,}])/g, '$1"$2"'));
        }
        catch {
    ...
    static async fetchCoinInfo(contractTxid, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
    ...
    static async getCoinbalance(contractTxid, addressOrHash, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        let hash = "";
    ...
    static async fetchCoinUTXOList(contractTxid, addressOrHash, codeScript, network) {
        let base_url = network
            ? API.getBaseURL(network)
            : API.getBaseURL("mainnet");
        let hash = "";
        ...
        const url = base_url + `stablecoin/utxo/combinescript/${hash}/stablecoinid/${contractTxid}`;
        const data = await API.coinData(url);
        if (!Array.isArray(data.utxos))
            throw new Error("Coin API success response is missing a UTXO array");
    ...
    static async fetchCoinUTXOs(contractTxid, addressOrHash, amount, codeScript, network, number) {
        ...
        try {
            const coinutxolist = await API.fetchCoinUTXOList(contractTxid, addressOrHash, codeScript, network);
            ...
            if (sumBalance < amount) {
                const totalBalance = coinutxolist.reduce((sum, utxo) => sum + BigInt(utxo.ftBalance), BigInt(0));
                if (totalBalance >= amount) {
                    if (number !== undefined) {
                        throw new Error("Insufficient Coinbalance within number limit, please merge Coin UTXOs or increase number");
                    }
