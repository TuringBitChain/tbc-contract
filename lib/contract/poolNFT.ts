...
class poolNFT {
  ...
  constructor(config?: {
    ...
    if (typeof config!.txidOrParams === "string") {
      ...
      if (
        config!.txidOrParams.ft_a <= 0 ||
        config!.txidOrParams.tbc_amount <= 0
      ) {
        throw new Error("Invalid number.");
      }
      this.ft_a_amount = BigInt(0);
      this.tbc_amount = BigInt(
        Math.floor(config!.txidOrParams.tbc_amount * Math.pow(10, 6))
      );
  ...
  async createPoolNFT(
    ...
    const poolnftTapeScript = tbc.Script.fromASM(
      ...
      } ${amountData} ${this.ft_a_contractTxid} 4e54617065`
    );
  ...
  async createPoolNftWithLock(
    ...
    FTA.initialize(FTAInfo);
    //const utxo = await API.fetchUTXO(privateKey, 0.1, this.network);
    this.poolnft_code = this.getPoolNftCodeWithLock(txSource.hash, 0)
      .toBuffer()
      .toString("hex");
    const ftlpCode = this.getFTLPcode(
      tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString(
  ...
  async consumeLP(
    ...
    tx.to(
      privateKey.toAddress().toString(),
      Number(changeDate.tbc_amount_difference)
    );
    //FTLP_Burn
    const nameHex = Buffer.from(FTA.name, "utf8").toString("hex");
    const symbolHex = Buffer.from(FTA.symbol, "utf8").toString("hex");
    ...
    // FTAbyC_change
    if (
      changeDate.ft_a_difference < tapeAmountSum ||
      fttxo_c.satoshis > Number(changeDate.tbc_amount_difference)
    ) {
      const ftabycCodeScript = FT.buildFTtransferCode(
        FTA.codeScript,
        poolnft_codehash160
  ...
  async swaptoTBC(
    ...
    this.tbc_amount = BigInt(this.tbc_amount) - BigInt(amount_tbcbn);
    this.ft_a_amount = BigInt(poolMul) / BigInt(this.tbc_amount);
    const ft_a_amount_increment =
      BigInt(this.ft_a_amount) - BigInt(ft_a_amount);
    const tapeAmountSetIn: bigint[] = [];
    const ftPreTX: tbc.Transaction[] = [];
    const ftPrePreTxData: string[] = [];
    // Fetch FT UTXO for the transfer
    ...
    tapeAmountSetIn.push(fttxo_a.ftBalance!);
    const ftutxo_codeScript_c = FT.buildFTtransferCode(
      FTA.codeScript,
      poolnft_codehash160
    )
      .toBuffer()
    ...
    // Build the amount and change hex strings for the tape
    const { amountHex, changeHex } = FT.buildTapeAmount(
      BigInt(ft_a_amount_increment) + BigInt(fttxo_c.ftBalance!),
      tapeAmountSetIn,
      1
    );
    //const utxo = await API.fetchUTXO(privateKey, 0.1, this.network);
  ...
  async swaptoTBC_baseToken(
    ...
    // Build the amount and change hex strings for the tape
    const { amountHex, changeHex } = FT.buildTapeAmount(
      BigInt(amount_ftbn) + BigInt(fttxo_c.ftBalance!),
      tapeAmountSetIn,
      1
    );
    //const utxo = await FTA.fetchUTXO(privateKey.toAddress().toString());
  ...
  async mergeFTinPool(
    ...
    const url_testnet = `http://192.168.50.109:18081/api/tbc/ft/utxo/combinescript/${hash}/contract/${contractTxid}`;
    const url_mainnet = `http://192.168.50.109:18081/api/tbc/ft/utxo/combinescript/${hash}/contract/${contractTxid}`;
    let url = "";
    if (this.network == "testnet") {
      url = url_testnet;
    } else if (this.network == "mainnet") {
      ...
      url =
        (this.network.endsWith("/") ? this.network : this.network + "/") +
        `ft/utxo/combinescript/${hash}/contract/${contractTxid}`;
    }
    const fttxo_codeScript = FT.buildFTtransferCode(
      FTA.codeScript,
      poolnft_codehash160
    )
      .toBuffer()
    ...
    try {
      ...
      const contractTX = await API.fetchTXraw(poolnft.txId, this.network);
      //const utxo = await API.fetchUTXO(privateKey, 0.1, this.network);
      const tx = new tbc.Transaction().from(poolnft).from(fttxo).from(utxo);
      //poolNft
      tx.addOutput(
        new tbc.Transaction.Output({
          script: tbc.Script.fromHex(this.poolnft_code),
  ...
  async getPoolNFTunlock(
    ...
    const currenttxoutputsdata = getCurrentTxOutputsDataforPool1(
      currentTX,
      option,
      swapOption
    );
    const sig =
      (currentTX.getSignature(currentUnlockIndex, privateKey).length / 2)
        .toString(16)
        .padStart(2, "0") +
      currentTX.getSignature(currentUnlockIndex, privateKey);
    const publicKey =
  ...
  private updateWhenTbcAmountChange(incrementBN: bigint) {
    ...
    if (increment == BigInt(0)) {
      ...
      const ratio =
        (BigInt(this.tbc_amount) * BigInt(this.precision)) / increment;
