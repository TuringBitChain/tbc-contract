...
class FT {
     ...
     */
    getFTunlock(privateKey_from, currentTX, preTX, prepreTxData, currentUnlockIndex, preTxVout, isCoin) {
        const privateKey = privateKey_from;
     ...
     * @returns The unlocking script as a tbc.Script object.
     */
    getFTunlockSwap(privateKey_from, currentTX, preTX, prepreTxData, contractTX, currentUnlockIndex, preTxVout, ftVersion, isCoin, isContractTXs) {
        const privateKey = privateKey_from;
        const prepretxdata = prepreTxData;
        const resolvedFtVersion = ftVersion ?? 1;
        const contracttxdata = (0, ftunlock_1.getContractTxdata)(contractTX, resolvedFtVersion > 1 ? -1 : currentTX.inputs[0].outputIndex);
        const pretxdata = (0, ftunlock_1.getPreTxdata)(preTX, preTxVout);
        ...
        const unlockingScript = resolvedFtVersion > 2
            ? new tbc.Script(`${currenttxdata}${prepretxdata}${sig}${publicKey}${currentinputsdata}${contracttxdata}${inputIndexMap[isContractTXs ? currentUnlockIndex - 1 : 0]}${isCoin ? "51" : ""}${pretxdata}`)
            : new tbc.Script(`${currenttxdata}${prepretxdata}${sig}${publicKey}${currentinputsdata}${contracttxdata}${isCoin ? "51" : ""}${pretxdata}`);
        return unlockingScript;
    }
    static getFTunlock(sigs, pubKey, currentTX, preTX, prepreTxData, currentUnlockIndex, preTxVout, isCoin) {
        const prepretxdata = prepreTxData;
        const pretxdata = (0, ftunlock_1.getPreTxdata)(preTX, preTxVout);
        const currenttxdata = (0, ftunlock_1.getCurrentTxdata)(currentTX, currentUnlockIndex);
    ...
    static getFTunlockSwap(sigs, pubKey, currentTX, preTX, prepreTxData, contractTX, currentUnlockIndex, preTxVout, ftVersion, isCoin, isContractTXs) {
        const prepretxdata = prepreTxData;
        const resolvedFtVersion = ftVersion ?? 1;
        const contracttxdata = (0, ftunlock_1.getContractTxdata)(contractTX, resolvedFtVersion > 1 ? -1 : currentTX.inputs[0].outputIndex);
        const pretxdata = (0, ftunlock_1.getPreTxdata)(preTX, preTxVout);
     ...
     */
    static buildMultiTapeAmounts(outputAmounts, tapeAmountSetIn) {
