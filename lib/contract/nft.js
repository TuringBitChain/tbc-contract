...
class NFT {
    ...
    initialize(nftInfo) {
        const { collectionId, collectionIndex, collectionName, nftName, nftSymbol, nft_attributes, nftDescription, nftTransferTimeCount, } = nftInfo;
    ...
    static batchCreateNFT(collection_id, address, privateKey, datas, utxos, nfttxos) {
        ...
        return txs.map((tx) => ({ txraw: tx.uncheckedSerialize() }));
    }
    /**
    ...
    transferNFT_v1(address_from, address_to, privateKey, utxos, pre_tx, pre_pre_tx, batch = false) {
        const code = NFT.buildCodeScript_v1(this.collection_id, this.collection_index);
        const tx = new tbc.Transaction()
            .addInputFromPrevTx(pre_tx, 0)
            .addInputFromPrevTx(pre_tx, 1)
            .from(utxos)
    ...
    transferNFTWithTBC(address_from, address_to_nft, address_to_tbc, privateKey, utxos, pre_tx, pre_pre_tx, tbc_amount) {
        const code = NFT.buildCodeScript(this.collection_id, this.collection_index);
    ...
    transferNFTWithTBC_v1(address_from, address_to_nft, address_to_tbc, privateKey, utxos, pre_tx, pre_pre_tx, tbc_amount) {
        const code = NFT.buildCodeScript_v1(this.collection_id, this.collection_index);
        const amount_satoshis = Number((0, util_1.parseDecimalToBigInt)(tbc_amount, 6));
        const tx = new tbc.Transaction()
            .addInputFromPrevTx(pre_tx, 0)
    ...
    static buildUnlockScript(privateKey_from, currentTX, preTX, prepreTxData, currentUnlockIndex) {
    ...
    static buildCodeScript_v1(tx_hash, outputIndex) {
        const tx_id = Buffer.from(tx_hash, "hex").reverse().toString("hex");
        const writer = new tbc.encoding.BufferWriter();
        const vout = writer.writeUInt32LE(outputIndex).toBuffer().toString("hex");
        const tx_id_vout = "0x" + tx_id + vout;
    ...
    static getNFTVersion(codeScript) {
        try {
            const codeBuffer = typeof codeScript === "string"
