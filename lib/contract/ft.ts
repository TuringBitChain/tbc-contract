...
class FT {
    ...
    static getFTunlockSwap(sigs: string, pubKey: string, currentTX: tbc.Transaction, preTX: tbc.Transaction, prepreTxData: string, contractTX: tbc.Transaction, currentUnlockIndex: number, preTxVout: number, newFT?: -1): tbc.Script {
        const prepretxdata = prepreTxData;
        const contracttxdata = getContractTxdata(contractTX, newFT | currentTX.inputs[0].outputIndex);
        const pretxdata = getPreTxdata(preTX, preTxVout);
        const currentinputsdata = getCurrentInputsdata(currentTX);
        const currenttxdata = getCurrentTxdata(currentTX, currentUnlockIndex);
        const signature = sigs;
