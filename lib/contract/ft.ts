...
class FT {
     ...
     * @returns The unlocking script as a tbc.Script object.
     */
    getFTunlockSwap(privateKey_from: tbc.PrivateKey, currentTX: tbc.Transaction, preTX: tbc.Transaction, prepreTxData: string, contractTX: tbc.Transaction, currentUnlockIndex: number, preTxVout: number, ftVersion?: 1 | 2 | 3, isCoin?: boolean, isContractTXs?: boolean): tbc.Script {
        const privateKey = privateKey_from;
        const prepretxdata = prepreTxData;
    ...
    static getFTunlockSwap(sigs: string, pubKey: string, currentTX: tbc.Transaction, preTX: tbc.Transaction, prepreTxData: string, contractTX: tbc.Transaction, currentUnlockIndex: number, preTxVout: number, ftVersion?: 1 | 2 | 3, isCoin?: boolean, isContractTXs?: boolean): tbc.Script {
        const prepretxdata = prepreTxData;
        const resolvedFtVersion = ftVersion ?? 1;
        const contracttxdata = getContractTxdata(contractTX, resolvedFtVersion > 1 ? -1 : currentTX.inputs[0].outputIndex);
        const pretxdata = getPreTxdata(preTX, preTxVout);
