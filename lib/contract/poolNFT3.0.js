...
const ftlpTbc20unlock_1 = require("../util/poolnft3/ftlpTbc20unlock");
const tbc20unlock_1 = require("../util/tbc20/tbc20unlock");
...
class PoolNFT3 {
    ...
    quoteAddLP(poolTx, incrementSat, firstFtAmountRaw) {
        const state = this.readPoolState(poolTx);
        return {
            ...(0, math_1.quoteAddLP)(state, incrementSat, firstFtAmountRaw),
    ...
    payment(address, value, dust = 42n) {
        if (value < dust)
            (0, transaction_1.pool3Fail)('required TBC payout is below dust');
    ...
    prepareAddLP(options) {
        const state = this.readPoolState(options.pool.parentTx);
        const quote = (0, math_1.quoteAddLP)(state, options.incrementSat, options.firstFtAmountRaw);
