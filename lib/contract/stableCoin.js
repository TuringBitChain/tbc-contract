...
const ftunlock_1 = require("../util/ft/ftunlock");
const nftunlock_1 = require("../util/nft/nftunlock");
const util_1 = require("../util/common/util");
const FT = require("./ft");
const NFT = require("./nft");
const SIGHASH_ALL_FORKID = tbc.crypto.Signature.SIGHASH_ALL |
    tbc.crypto.Signature.SIGHASH_FORKID;
function computeInputSighash(tx, inputIndex) {
...
function encodeSchnorrSig65Hex(sig64) {
    if (!Buffer.isBuffer(sig64) || sig64.length !== 64) {
        throw new Error("Schnorr signature must be 64 bytes");
    }
    return Buffer.concat([sig64, Buffer.from([SIGHASH_ALL_FORKID])]).toString("hex");
}
 ...
 */
function preseedAdminInputsAndFreezeFee(tx, feePrivateKey, adminUnlockBuilders) {
    for (const { inputIndex, buildWithSig } of adminUnlockBuilders) {
        tx.setInputScript({ inputIndex }, (t) => buildWithSig(DUMMY_SCHNORR_SIG64, t));
    }
...
class stableCoin extends FT {
    initialize(info) {
        ...
        try {
            legacyCoin = (0, ftscript_1.isCoinCodeScript)(info.codeScript);
        }
        catch { /* Invalid legacy Code. */ }
        if (!legacyCoin) {
    ...
    transferWithAdditionalInfo(key, recipient, amount, utxos, fee, parents, proofs, data) {
        return this.transfer(key, recipient, amount, utxos, fee, parents, proofs, undefined, data);
    }
    mergeFT(key, utxos, fee, parents, proofs, localTX = []) {
    ...
    buildIssuanceScripts(adminPubHashHex, receiveAddress, issuerCodeHash, amountRaw) {
        ...
        const tapeScript = tbc.Script.fromASM(`OP_FALSE OP_RETURN ${tapeAmount} ${decimalHex} ${nameHex} ${symbolHex} 00000000 4654617065`);
        return {
            codeScript: stableCoin.getCoinMintCode(adminPubHashHex, receiveAddress, issuerCodeHash, tapeScript.toBuffer().length),
            tapeScript,
        };
    }
    /**
     ...
     */
    createCoin(aggPubkey32, feePrivateKey, address_to, utxo, utxoTX, mintMessage) {
        ...
        const coinNftTX = stableCoin.buildCoinNftTX(feePrivateKey, adminPubHash, utxo, data);
        ...
        const { codeScript, tapeScript } = this.buildIssuanceScripts(adminPubHash, address_to, originCodeHash, totalSupply);
        ...
        preseedAdminInputsAndFreezeFee(tx, feePrivateKey, [
            {
                inputIndex: 0,
                buildWithSig: (sig64, t) => coinNft.buildUnlockScriptSchnorr(sig64, aggPubkey32, t, coinNftTX, utxoTX, 0),
            },
            {
                inputIndex: 1,
                buildWithSig: (sig64) => tbc.Script.fromASM(`${encodeSchnorrSig65Hex(sig64)} ${aggPubkey32.toString("hex")}`),
            },
        ]);
        const sighashes = [
        ...
        const self = this;
        const finalize = (schnorrSigs64) => {
            if (!Array.isArray(schnorrSigs64) || schnorrSigs64.length !== 2) {
                throw new Error("createCoin.finalize: expected 2 Schnorr sigs");
            }
            tx.setInputScript({ inputIndex: 0 }, (t) => coinNft.buildUnlockScriptSchnorr(schnorrSigs64[0], aggPubkey32, t, coinNftTX, utxoTX, 0)).setInputScript({ inputIndex: 1 }, () => tbc.Script.fromASM(`${encodeSchnorrSig65Hex(schnorrSigs64[1])} ${aggPubkey32.toString("hex")}`));
     ...
     */
    mintCoin(aggPubkey32, feePrivateKey, address_to, mintAmount, utxo, nftPreTX, nftPrePreTX, mintMessage) {
        if (!Buffer.isBuffer(aggPubkey32) || aggPubkey32.length !== 32) {
            throw new Error("aggPubkey32 must be 32 bytes (x-only)");
        }
        const adminPubHash = hash160Hex(aggPubkey32);
        ...
        const originCodeHash = tbc.crypto.Hash.sha256(coinNftTX.outputs[0].script.toBuffer()).toString("hex");
        const { codeScript, tapeScript } = this.buildIssuanceScripts(adminPubHash, address_to, originCodeHash, newMintAmount);
        this.codeScript = codeScript.toBuffer().toString("hex");
        this.tapeScript = tapeScript.toBuffer().toString("hex");
        ...
        preseedAdminInputsAndFreezeFee(tx, feePrivateKey, [
            {
                inputIndex: 0,
                buildWithSig: (sig64, t) => coinNft.buildUnlockScriptSchnorr(sig64, aggPubkey32, t, nftPreTX, nftPrePreTX, 0),
            },
            {
                inputIndex: 1,
                buildWithSig: (sig64) => tbc.Script.fromASM(`${encodeSchnorrSig65Hex(sig64)} ${aggPubkey32.toString("hex")}`),
        ...
        const finalize = (schnorrSigs64) => {
            ...
            tx.setInputScript({ inputIndex: 0 }, (t) => coinNft.buildUnlockScriptSchnorr(schnorrSigs64[0], aggPubkey32, t, nftPreTX, nftPrePreTX, 0)).setInputScript({ inputIndex: 1 }, () => tbc.Script.fromASM(`${encodeSchnorrSig65Hex(schnorrSigs64[1])} ${aggPubkey32.toString("hex")}`));
            tx.seal();
     ...
     */
    transfer(privateKey_from, address_to, ft_amount, ftutxo_a, utxo, preTX, prepreTxData, tbc_amount, additionalInfo) {
        ...
        if (additionalInfo !== undefined) {
            if (!Buffer.isBuffer(additionalInfo))
                throw new Error("additionalInfo must be a Buffer");
            tx.addOutput(new tbc.Transaction.Output({
     ...
     */
    batchTransfer_old(privateKey_from, receiveAddressAmount, ftutxo, utxo, preTX, prepreTxData) {
        const privateKey = privateKey_from;
        let txsraw = [];
    ...
    _batchTransfer_old(privateKey_from, address_to, amount, preTX, prepreTxData, txsraw, ftutxoBalance, ftutxo, utxo) {
        const privateKey = privateKey_from;
        const address_from = privateKey.toAddress().toString();
        const code = this.codeScript;
        const tape = this.tapeScript;
     ...
     * @returns {Array<{ txraw: string }>} Returns an array containing unchecked transaction raw data.
     */
    batchTransfer(privateKey_from, receivers, ftutxo, utxo, preTX, prepreTxData) {
        const privateKey = privateKey_from;
        const txsraw = [];
        let ftutxoBalance = 0n;
        ...
        for (let b = 0; b < batches.length; b++) {
            ...
            for (const receiver of batch) {
                ftutxoBalance -= (0, util_1.parseDecimalToBigInt)(receiver.amount, this.decimal);
            }
        }
        return txsraw;
    }
    _batchTransfer(privateKey_from, receivers, preTX, prepreTxData, txsraw, ftutxoBalance, ftutxo, utxo, prevBatchSize) {
        const privateKey = privateKey_from;
        const address_from = privateKey.toAddress().toString();
        const code = this.codeScript;
        const tape = this.tapeScript;
        ...
        for (const receiver of receivers) {
            if ((typeof receiver.amount === "string" && parseFloat(receiver.amount) < 0) ||
                (typeof receiver.amount === "number" && receiver.amount < 0)) {
                throw new Error("Invalid amount input");
            }
            const amountbn = (0, util_1.parseDecimalToBigInt)(receiver.amount, decimal);
        ...
        if (ftutxo) {
            for (let i = 0; i < ftutxo.length; i++) {
                ...
                tx.setInputScript({ inputIndex: i }, (tx) => {
                    return this.getFTunlock(privateKey, tx, preTX[i], prepreTxData[i], i, ftutxo[i].outputIndex, isCoin);
                });
            }
        }
        else {
            ...
            tx.setInputScript({ inputIndex: 0 }, (tx) => {
                return this.getFTunlock(privateKey, tx, preTX[0], prepreTxData[0], 0, ftChangeIndex, isCoin);
            });
        }
        tx.sign(privateKey);
        tx.setLockTime(lockTimeMax);
    ...
    mergeCoin(privateKey_from, ftutxo, utxo, preTX, prepreTxData, localTX = []) {
        ...
        localTX = preTXs;
        const txs = this.mergeCoin(privateKey, ftutxos, newutxo, preTXs, prepreTxDatas, localTX);
     ...
     */
    freezeCoinUTXO(aggPubkey32, feePrivateKey, lock_time, ftutxo, utxo, preTX, prepreTxData) {
        if (!Buffer.isBuffer(aggPubkey32) || aggPubkey32.length !== 32) {
            throw new Error("aggPubkey32 must be 32 bytes (x-only)");
        }
        ...
        const adminBuilders = ftutxos.map((_, i) => ({
            inputIndex: i,
            buildWithSig: (sig64, t) => FT.getFTunlock(encodeSchnorrSig65Hex(sig64), xOnlyHex, t, preTX[i], prepreTxData[i], i, ftutxos[i].outputIndex, isCoin),
        }));
        ...
        const finalize = (schnorrSigs64) => {
            if (!Array.isArray(schnorrSigs64) || schnorrSigs64.length !== ftutxos.length) {
                throw new Error(`freezeCoinUTXO.finalize: expected ${ftutxos.length} Schnorr sigs, got ${schnorrSigs64?.length}`);
            }
            for (let i = 0; i < ftutxos.length; i++) {
                const sig65Hex = encodeSchnorrSig65Hex(schnorrSigs64[i]);
                const idx = i;
                tx.setInputScript({ inputIndex: idx }, (t) => FT.getFTunlock(sig65Hex, xOnlyHex, t, preTX[idx], prepreTxData[idx], idx, ftutxos[idx].outputIndex, isCoin));
            }
            tx.seal();
            return tx.uncheckedSerialize();
        };
     ...
     * @param feePrivateKey - ECDSA key funding the tx and signing the fee input.
     */
    unfreezeCoinUTXO(aggPubkey32, feePrivateKey, ftutxo, utxo, preTX, prepreTxData) {
        if (!Buffer.isBuffer(aggPubkey32) || aggPubkey32.length !== 32) {
            throw new Error("aggPubkey32 must be 32 bytes (x-only)");
        }
        const controlData = stableCoin.getAddressFromCode(ftutxo[0].script);
        ...
        const tapeScript = stableCoin.setLockTimeInTape(FT.buildFTtransferTape(this.tapeScript, amountHex), 0);
        tx.addOutput(new tbc.Transaction.Output({
            script: tapeScript,
            satoshis: 0,
        }));
        ...
        const finalize = (schnorrSigs64) => {
            if (!Array.isArray(schnorrSigs64) || schnorrSigs64.length !== ftutxos.length) {
                throw new Error(`unfreezeCoinUTXO.finalize: expected ${ftutxos.length} Schnorr sigs, got ${schnorrSigs64?.length}`);
            }
            for (let i = 0; i < ftutxos.length; i++) {
                const sig65Hex = encodeSchnorrSig65Hex(schnorrSigs64[i]);
                const idx = i;
     ...
     */
    static buildCoinNftTX(feePrivateKey, adminPubHashHex, utxo, data) {
        const feeAddress = feePrivateKey.toAddress().toString();
        const nftCodeScript = coinNft.getCoinNftCode(utxo.txId, utxo.outputIndex);
        const nftHoldScript = coinNft.getHoldScriptFromHash(adminPubHashHex, data.nftName);
...
class coinNft extends NFT {
    ...
    static getHoldScriptFromHash(pubKeyHashHex, flag) {
        ...
        const flagHex = Buffer.from(`For Coin ${flag} NHold`, "utf8").toString("hex");
        return tbc.Script.fromASM(`OP_DUP OP_HASH160 ${pubKeyHashHex} OP_EQUALVERIFY OP_CHECKSIG OP_RETURN ${flagHex}`);
    }
    /**
     ...
     */
    static buildUnlockScriptSchnorr(schnorrSig64, xOnlyPubkey32, currentTX, preTX, prepreTxData, currentUnlockIndex) {
        if (!Buffer.isBuffer(xOnlyPubkey32) || xOnlyPubkey32.length !== 32) {
            throw new Error("xOnlyPubkey32 must be 32 bytes");
        }
        const currenttxdata = (0, nftunlock_1.getCurrentTxdata)(currentTX);
        const prepretxdata = (0, nftunlock_1.getPrePreTxdata)(prepreTxData);
        const pretxdata = (0, nftunlock_1.getPreTxdata)(preTX);
    ...
    static decodeTapeScript(tapeScript) {
        const data = tapeScript.chunks[tapeScript.chunks.length - 2].buf.toString("utf8");
        const jsonData = JSON.parse(data);
        return jsonData;
    }
