...
class MultiSig {
  ...
  static batchSignMultiSigTransaction_sendTBC(
    ...
    for (let txIndex = 0; txIndex < multiSigTxraws.length; txIndex++) {
      const sigs = MultiSig.signMultiSigTransaction_sendTBC(
        address_from,
        multiSigTxraws[txIndex],
        privateKey
      );
      allSigs.push(sigs);
    }
    ...
    for (let j = 0; j < sigs.length; j++) {
      tx.setInputScript(
        ...
        (tx) => {
          ...
          const unlockingScript = tbc.Script.fromASM(
            `OP_0 ${signature} ${multiPubKeys}`
          );
  ...
  static batchFinishMultiSigTransaction_sendTBC(
    ...
    let finishedTxs: string[] = [];

    for (let txIndex = 0; txIndex < txraws.length; txIndex++) {
      const finishedTx = MultiSig.finishMultiSigTransaction_sendTBC(
        txraws[txIndex],
        sigs[txIndex],
        pubKeys
      );
  ...
  static batchSignMultiSigTransaction_transferFT(
    ...
    for (let txIndex = 0; txIndex < multiSigTxraws.length; txIndex++) {
      const sigs = MultiSig.signMultiSigTransaction_transferFT(
        multiSig_address,
        multiSigTxraws[txIndex],
        privateKey
      );
  ...
  static batchFinishMultiSigTransaction_transferFT(
    ...
    for (let txIndex = 0; txIndex < txraws.length; txIndex++) {
      const finishedTx = MultiSig.finishMultiSigTransaction_transferFT(
        txraws[txIndex],
        sigs[txIndex],
        pubKeys
      );
  ...
  static validateMultiSigAddress(address: string) {
  ...
  static getCombineHash(address: string): string {
    const combine_hash =
      tbc.crypto.Hash.sha256ripemd160(
        tbc.crypto.Hash.sha256(
          tbc.Script.fromASM(MultiSig.getMultiSigLockScript(address)).toBuffer()
        )
      ).toString("hex") + "01";
    return combine_hash;
  }
  ...
  private static buildHoldScript(pubKey: string): tbc.Script {
    const publicKeyHash = tbc.crypto.Hash.sha256ripemd160(
  ...
  private static buildTapeScript(
    address: string,
    pubKeys: string[]
  ): tbc.Script {
    const data = {
