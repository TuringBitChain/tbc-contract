...
const getPoolTapeScriptLockFlag = (
  poolTapeScript: tbc.Script | undefined,
  fallback: 0 | 1,
): 0 | 1 => {
  const lockFlag = getScriptChunkNumber(poolTapeScript?.chunks[7]);
  if (lockFlag === null) return fallback;
  return lockFlag === 1 ? 1 : 0;
};

const getPoolTapeLockFlag = (
  poolTX: tbc.Transaction,
  poolCodeVout: number,
  fallback: 0 | 1,
): 0 | 1 => {
  return getPoolTapeScriptLockFlag(
    poolTX.outputs[poolCodeVout + 1]?.script,
    fallback,
  );
};
...
class poolNFT2 {
  ...
  private getCachedPoolLockFlag(): 0 | 1 {
    return this.with_lock === true ? 1 : 0;
  }

  private async fetchPoolLockFlag(poolUtxoTxid?: string): Promise<0 | 1> {
    const poolnftTapeScript = await this.fetchPoolNftTape(poolUtxoTxid);
    const lockFlag = getPoolTapeScriptLockFlag(
      poolnftTapeScript,
      this.getCachedPoolLockFlag(),
    );
  ...
  async createPoolNFT(
    ...
    this.ft_lp_partialhash = partial_sha256.calculate_partial_hash(
      ftlpCode.toBuffer().subarray(0, ftlpOffset),
    );
  ...
  async initPoolNFT(
    ...
    let totalTbcAmount: bigint;
    let ftAAmount: bigint;
    try {
      totalTbcAmount = parseDecimalToBigInt(tbc_amount, 6);
      ftAAmount = parseDecimalToBigInt(ft_a, FTA.decimal);
    } catch {
      throw new Error("Invalid amount Input");
    }
    const lpCostAmount = lockStatus
      ? getLpCostAmount(this.poolnft_code)
      : 0;
    const poolTbcAmount = totalTbcAmount - BigInt(lpCostAmount);
    if (lockStatus && poolTbcAmount <= BigInt(0)) {
      const lpCostTBC = Number(
        (lpCostAmount / Math.pow(10, 6)).toFixed(6),
      );
      throw new Error(
    ...
    if (lockStatus) {
      const lpCostAddress = getLpCostAddress(this.poolnft_code);
  ...
  async consumeLP(
    ...
    const lockStatus = getPoolTapeLockFlag(
      contractTX,
      poolnft.outputIndex,
      this.getCachedPoolLockFlag(),
    );
   ...
   *
   */
  async swaptoTBC_baseToken_local(
    privateKey_from: tbc.PrivateKey,
    address_to: string,
    ...
    tx.setInputScript(
      ...
      (tx) => {
        const unlockingScript = this.getPoolNftUnlockOffLine(
          privateKey,
          tx,
          0,
          poolNftPreTX,
   ...
   */
  async getLpIncome(address: string, recordData: bigint): Promise<bigint> {
    ...
    const my_tbc_amount =
      (pool_tbc_amount_full * BigInt(this.precision)) / ratio;
  ...
  async _mergeFTinPool(
    ...
    try {
      ...
      await tx.setInputScriptAsync(
        ...
        async (tx) => {
          const unlockingScript = await this.getPoolNftUnlockOffLine(
            privateKey,
            tx,
            0,
            poolnftPreTX,
  ...
  getPoolNftUnlockOffLine(
    ...
    for (let i = 1; i < currentTX.inputs.length; i++) {
      ...
      if (option == 3) {
        currentinputstxdata =
          getInputsTxdataSwap(inputsTX, currentTX.inputs[i].outputIndex) +
          currentinputstxdata;
      } else {
    ...
    const currenttxoutputsdata = getCurrentTxOutputsDataforPool2(
      currentTX,
      option,
      withLock,
      swapOption,
    ...
    const poolTapeWithLock = getPoolTapeLockFlag(
      preTX,
      currentTX.inputs[currentUnlockIndex].outputIndex,
      withLock,
    );
    if (
      poolCodeLength > 3284 ||
      ...
      switch (option) {
        ...
        case 3:
          if (poolTapeWithLock) {
            ...
            const publicKey =
              (privateKey.toPublicKey().toString().length / 2)
                .toString(16)
  ...
  updatePoolNFT(
    ...
    if (this.tbc_amount > tbc_amount_old) {
      return {
        ...
        tbc_amount_full_difference:
          BigInt(this.tbc_amount_full) - BigInt(tbc_amount_full_old),
      };
    } else {
      return {
        ...
        tbc_amount_full_difference:
          BigInt(tbc_amount_full_old) - BigInt(this.tbc_amount_full),
      };
    }
  }
  ...
  private updateWhenFtLpChange(incrementBN: bigint) {
    ...
    if (increment == BigInt(0)) {
      ...
      const ratio =
        (BigInt(this.ft_lp_amount) * BigInt(this.precision)) / increment;
  ...
  private updateWhenFtAChange(incrementBN: bigint) {
    ...
    if (increment == BigInt(0)) {
      ...
      const ratio =
        (BigInt(this.ft_a_amount) * BigInt(this.precision)) / increment;
      ...
      const ratio =
        (BigInt(increment) * BigInt(this.precision)) / BigInt(this.ft_a_amount);
      this.ft_a_amount = BigInt(this.ft_a_amount) + BigInt(increment);
      this.ft_lp_amount =
        BigInt(this.ft_lp_amount) +
  ...
  private updateWhenTbcAmountChange(incrementBN: bigint) {
    ...
    if (increment == BigInt(0)) {
      ...
      const ratio =
        (BigInt(increment) * BigInt(this.precision)) /
        (BigInt(this.tbc_amount_full) - BigInt(this.poolnft_code_dust));
      this.tbc_amount = BigInt(this.tbc_amount) + BigInt(increment);
      this.ft_lp_amount =
        BigInt(this.ft_lp_amount) +
  ...
  getPoolNftCode(
    ...
    const ftCodeSize =
      ftVersion === 4 ? "1c08" : isCoin ? "dc07" : ftVersion === 1 ? "1c06" : "5c07";
  ...
  getPoolNftCodeWithLock(
    ...
    if (lpCostTBC <= 0) throw new Error("lpCostTBC must be greater than 0");
    ...
    const lpCostAddressHex =
      lpCostAddress instanceof tbc.Address
    ...
    const code = tbc.Script.fromString(
      poolNftCodePre.toString() +
        " " +
        firstCode.toString() +
        " " +
        lastCode.toString() +
  ...
  getFtlpCode(
    ...
    const ftlpCodeLast = buildFtlpCodeLast(
      hash,
      getFtlpTargetLength(isCoin, ftVersion),
      ftlpCodePre,
      shouldUseFtlpPushData2(isCoin, ftVersion),
    );
...
const replaceFtlpCodePreFragment = (
  source: string,
  searchValue: string,
  replaceValue: string,
  ...
  if (actualCount !== expectedCount) {
    throw new Error(
      `Invalid FTLP code pre template: expected ${expectedCount} occurrences, got ${actualCount}.`,
    );
  }
  return source.split(searchValue).join(replaceValue);
};

const toLegacyFtlpCodePre = (source: string): string => {
  let legacySource = source;
  legacySource = replaceFtlpCodePreFragment(
    legacySource,
...
const getFtlpTargetLength = (
  isCoin: boolean,
  ftVersion?: FTVersion,
): number => {
  if (ftVersion === 4) return FT_V4_CODE_LENGTH;
  if (isCoin) return LEGACY_COIN_CODE_LENGTH;
  if (ftVersion === 2 || ftVersion === 3) return FT_V2_CODE_LENGTH;
  return FT_V1_CODE_LENGTH;
};

const shouldUseFtlpPushData2 = (
  isCoin: boolean,
  ftVersion?: FTVersion,
): boolean => {
...
const buildFtlpCodeLast = (
  hash: string,
  targetLength: number,
  ftlpCodePre: tbc.Script,
  usePushData2: boolean,
): tbc.Script => {
  const pushDataOverhead = usePushData2 ? 33 : 32;
  const paddingSize =
    targetLength - ftlpCodePre.toBuffer().length - pushDataOverhead;
  if (paddingSize < 0) {
    throw new Error(
      `Invalid FTLP code length: target ${targetLength}, pre ${ftlpCodePre.toBuffer().length}.`,
    );
  }
  ...
  return tbc.Script.fromString(
    `${usePushData2 ? "OP_PUSHDATA2" : "OP_PUSHDATA1"} 0x${paddingSizeHex} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`,
  );
};
...
interface poolNFTDifference {
  ...
  tbc_amount_full_difference: bigint;
}
