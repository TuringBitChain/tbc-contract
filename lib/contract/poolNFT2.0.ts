...
class poolNFT2 {
     ...
     */
    async createPoolNFT(privateKey_from: tbc.PrivateKey, utxo: tbc.Transaction.IUnspentOutput, tag: string, serviceFeeRate?: number, lpPlan?: 1 | 2, withLockTime?: boolean): Promise<string[]> {
        ...
        this.poolnft_code = this.getPoolNftCode(txSource.hash, 0, lpPlan || 1, tag).toBuffer().toString('hex');
     ...
     * 8. 封装交易并返回序列化后的未检查交易数据以供发送。
     */
    async createPoolNftWithLock(privateKey_from: tbc.PrivateKey, utxo: tbc.Transaction.IUnspentOutput, tag: string, lpCostAddress: tbc.Address | string, lpCostTBC: number, pubKeyLock: string[], serviceFeeRate?: number, lpPlan?: 1 | 2, withLockTime?: boolean): Promise<string[]> {
        const privateKey = privateKey_from;
        const publicKeyHash = tbc.Address.fromPrivateKey(privateKey).hashBuffer.toString('hex');
        const flagHex = Buffer.from('for poolnft mint', 'utf8').toString('hex');
        const txSource = new tbc.Transaction()//Build transcation
        ...
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
        this.poolnft_code = this.getPoolNftCodeWithLock(txSource.hash, 0, lpPlan || 1, lpCostAddress, lpCostTBC, pubKeyLock, tag).toBuffer().toString('hex');
        const ftlpCode = (withLockTime ?? false)
            ? this.getFtlpCodeWithLockTime(
            tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, 'hex')).toString('hex'),
            privateKey.toAddress().toString(),
     ...
     * 11. 封装交易并返回序列化后的未检查交易数据以供发送。
     */
    async initPoolNFTWithLockTime(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, tbc_amount: number, ft_a: number, lock_time: number): Promise<string> {
        const privateKey = privateKey_from;
        const FTA = new FT(this.ft_a_contractTxid);
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
    ...
    async increaseLP(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_tbc: number): Promise<string> {
        const lockStatus = this.with_lock === true ? 1 : 0 || isLock(this.poolnft_code.length);
     ...
     * 10. 封装交易并返回序列化后的未检查交易数据以供发送。
     */
    async increaseLpWithLockTime(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_tbc: number, lock_time: number): Promise<string> {
        const lockStatus = this.with_lock === true ? 1 : 0 || isLock(this.poolnft_code.length);
        // console.log(`Lock status: ${lockStatus}`);
        if (lockStatus) amount_tbc -= 5;
        if (amount_tbc <= 0) throw new Error('Invalid TBC amount input');
    ...
    async consumeLP(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_lp: number): Promise<string> {
        ...
        await tx.setInputScriptAsync({
            ...
            const unlockingScript = await this.getPoolNftUnlock(privateKey, tx, 0, poolnft.txId, poolnft.outputIndex, isLock(this.poolnft_code.length), 2);
            return unlockingScript;
        });
        await tx.setInputScriptAsync({
            inputIndex: 1,
     ...
     */
    async swaptoToken_baseTBC(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_tbc: number, lpPlan?: 1 | 2): Promise<string> {
        const privateKey = privateKey_from;
        const FTA = new FT(this.ft_a_contractTxid);
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
        lpPlan = (this.lp_plan === 1 || this.lp_plan === 2) ? this.lp_plan as 1 | 2 : (lpPlan || 1);
        if (amount_tbc <= 0) {
        ...
        const serviceFeeLP = amount_tbcbn * BigInt(lpPlan === 1 ? this.service_fee_rate : 5) / BigInt(10000);
        ...
        if (serviceFeeA >= BigInt(42)) {
            tx.to(lpPlan === 1 ? "13oCEJaqyyiC8iRrfup6PDL2GKZ3xQrsZL" : "1Fa6Uy64Ub4qNdB896zX2pNMx4a8zMhtCy", Number(serviceFeeA));
        }
        // FTAbyC_Change
        ...
        await tx.setInputScriptAsync({
            ...
            const unlockingScript = await this.getPoolNftUnlock(privateKey, tx, 0, poolnft.txId, poolnft.outputIndex, isLock(this.poolnft_code.length), 3, 1);
            return unlockingScript;
        });
        for (let i = 0; i < fttxo_c.length; i++) {
            await tx.setInputScriptAsync({
     ...
     * 9. 封装交易并返回序列化后的未检查交易数据以供发送。
     */
    async swaptoTBC_baseToken(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_token: number, lpPlan?: 1 | 2): Promise<string> {
        const privateKey = privateKey_from;
        const FTA = new FT(this.ft_a_contractTxid);
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
        ...
        this.ft_a_amount = BigInt(new BN(this.ft_a_amount.toString()).add(new BN(amount_ftbn.toString())).toString());
        ...
        const serviceFee = tbc_amount_decrement * BigInt(this.service_fee_rate + 10) / BigInt(10000);
        const serviceFeeLP = tbc_amount_decrement * BigInt(lpPlan === 1 ? this.service_fee_rate : 5) / BigInt(10000);
        const serviceFeeA = serviceFee - serviceFeeLP;
        ...
        await tx.setInputScriptAsync({
            ...
            const unlockingScript = await this.getPoolNftUnlock(privateKey, tx, 0, poolnft.txId, poolnft.outputIndex, isLock(this.poolnft_code.length), 3, 2);
            return unlockingScript;
        });
        await tx.setInputScriptAsync({
            inputIndex: 1,
    ...
    async swaptoTBC_baseToken_local(privateKey_from: tbc.PrivateKey, address_to: string, ftutxo: tbc.Transaction.IUnspentOutput, ftPreTX: tbc.Transaction[], ftPrePreTxData: string[], amount_token: number, lpPlan?: 1 | 2, utxo?: tbc.Transaction.IUnspentOutput): Promise<string> {
        ...
        tx.setInputScript({
            ...
            const unlockingScript = this.getPoolNftUnlockOffLine(privateKey, tx, 0, poolNftPreTX, poolNftPrePreTX, inputsTXs, isLock(this.poolnft_code.length), 3, 2);
            return unlockingScript;
        });
        tx.setInputScript({
            inputIndex: 1,
     ...
     * @param {boolean} [withLockTime=false] - 是否包含锁定时间的FTLP代码。
     ...
     * 11. 封装交易并返回序列化后的未检查交易数据以供发送。
     */
    async consumeLpWithLockTime(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_lp: number): Promise<string> {
        const privateKey = privateKey_from;
        const FTA = new FT(this.ft_a_contractTxid);
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
     ...
     * @deprecated 此方法已弃用，请勿使用。
     */
    async lockFTLP(privateKey_from: tbc.PrivateKey, utxo: tbc.Transaction.IUnspentOutput, lock_time: number): Promise<string> {
        const FTA = new FT(this.ft_a_contractTxid);
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        FTA.initialize(FTAInfo);
        const privateKey = privateKey_from;
        const address = privateKey.toAddress().toString();
        const ftlpCodeScript = this.getFtlpCodeWithLockTime(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, 'hex')).toString('hex'), address, FTA.tapeScript.length / 2);
        ...
        const ftutxo_codeScript = ftlpCodeScript.toBuffer().toString('hex');
        try {
            const response = await (await fetch(url)).json();
    ...
    async mergeFTLP(privateKey_from: tbc.PrivateKey, utxo: tbc.Transaction.IUnspentOutput): Promise<boolean | string> {
        ...
        const ftlpCodeHash = tbc.crypto.Hash.sha256(ftlpCode).reverse().toString('hex');
        const url_testnet = `https://tbcdev.org/v1/tbc/main/ft/lp/unspent/by/script/hash${ftlpCodeHash}`;
        const url_mainnet = `https://turingwallet.xyz/v1/tbc/main/ft/lp/unspent/by/script/hash${ftlpCodeHash}`;
        let url = "";
        if (this.network == "testnet") {
        ...
        const ftutxo_codeScript = ftlpCode.toString('hex');
        try {
            const response = await (await fetch(url)).json();
            ...
            tx.sign(privateKey);
            if (this.with_lock_time) tx.setLockTime((await API.fetchBlockHeaders(this.network))[0].height - 2);
            await tx.sealAsync();
            const txraw = tx.uncheckedSerialize();
     ...
     * @returns {Promise<Array<{ txHex: string }>>} 返回一个 Promise，解析为包含交易原始数据的数组。
     * @throws {Error} 如果请求失败或未能找到足够的 UTXO，将抛出错误。
     */
    async mergeFTinPool(privateKey_from: tbc.PrivateKey, utxo: tbc.Transaction.IUnspentOutput, times?: number): Promise<Array<{ txHex: string }>> {
        const privateKey = privateKey_from;
        try {
            const FTA = new FT(this.ft_a_contractTxid);
            const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
     ...
     */
    async _mergeFTinPool(privateKey: tbc.PrivateKey, poolnftPreTX: tbc.Transaction, poolnftPrePreTX: tbc.Transaction, ftutxos: tbc.Transaction.IUnspentOutput[], FTA: InstanceType<typeof FT>, txsraw: Array<{ txHex: string }>, utxo?: tbc.Transaction.IUnspentOutput): Promise<tbc.Transaction> {
        const poolnft: tbc.Transaction.IUnspentOutput = {
        ...
        try {
            ...
            await tx.setInputScriptAsync({
                ...
                const unlockingScript = await this.getPoolNftUnlockOffLine(privateKey, tx, 0, poolnftPreTX, poolnftPrePreTX, inputsTXs, isLock(this.poolnft_code.length), 4);
                return unlockingScript;
            });
     ...
     * 4. 返回生成的解锁脚本。
     */
    getPoolNftUnlockOffLine(privateKey_from: tbc.PrivateKey, currentTX: tbc.Transaction, currentUnlockIndex: number, poolnftPreTX: tbc.Transaction, poolnftPrePreTX: tbc.Transaction, inputsTXs: tbc.Transaction[], withLock: 0 | 1, option: 1 | 2 | 3 | 4, swapOption?: 1 | 2): tbc.Script {
        const privateKey = privateKey_from;
     ...
     * 4. 返回生成的解锁脚本。
     */
    async getPoolNftUnlock(privateKey_from: tbc.PrivateKey, currentTX: tbc.Transaction, currentUnlockIndex: number, preTxId: string, preVout: number, withLock: 0 | 1, option: 1 | 2 | 3 | 4, swapOption?: 1 | 2): Promise<tbc.Script> {
        const privateKey = privateKey_from;
    ...
    private updateWhenTbcAmountChange(incrementBN: bigint) {
        ...
        if (increment == BigInt(0)) {
            ...
            const ratio = ((BigInt(this.tbc_amount_full) - BigInt(this.poolnft_code_dust)) * BigInt(this.precision)) / increment;
            ...
            const ratio = (BigInt(increment) * BigInt(this.precision)) / (BigInt(this.tbc_amount_full) - BigInt(this.poolnft_code_dust));
            this.tbc_amount = BigInt(this.tbc_amount) + BigInt(increment);
            this.ft_lp_amount = BigInt(this.ft_lp_amount) + BigInt(this.ft_lp_amount) * ratio / BigInt(this.precision);
    ...
    private getPoolNftTape(lpPlan: 1 | 2, withLock?: boolean, withLockTime?: boolean): tbc.Script {
        ...
        const withLockTimeHex = (withLockTime ? 1 : 0).toString(16).padStart(2, '0');
    ...
    async getPoolNftExtraInfo() {
        ...
        const extraInfo = {
            serviceFeeRate: parseInt(poolnftTapeScript.chunks[5].buf.toString('hex'), 16),
            lpPlan: parseInt(poolnftTapeScript.chunks[6].buf.toString('hex'), 16),
            withLock: parseInt(poolnftTapeScript.chunks[7].buf.toString('hex'), 16) === 1,
            withLockTime: parseInt(poolnftTapeScript.chunks[8].buf.toString('hex'), 16) === 1,
        };
        return extraInfo;
    }
    
    getPoolNftCode(txid: string, vout: number, lpPlan: 1 | 2, tag?: string): tbc.Script {
        const writer = new tbc.encoding.BufferWriter();
    ...
    getPoolNftCodeWithLock(txid: string, vout: number, lpPlan: 1 | 2, lpCostAddress: tbc.Address | string, lpCostTBC: number, pubKeyLock: string[], tag?: string): tbc.Script {
        ...
        // switch (pubKeyLock.length) {
        //     case 1:
        //         lastCode = tbc.Script.fromASM(`${pubKeyLock[0]} OP_EQUALVERIFY OP_CHECKSIGVERIFY`);
        //         break;
        //     case 2:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[1]} OP_EQUALVERIFY OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 3:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[2]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 4:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[2]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[3]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 5:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[2]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[3]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[4]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 6:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[2]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[3]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[4]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[5]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 7:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[2]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[3]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[4]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[5]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[6]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        //     case 8:
        //         lastCode = tbc.Script.fromASM(`OP_DUP ${pubKeyLock[0]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[1]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[2]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[3]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[4]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[5]} OP_EQUAL OP_IF OP_DROP OP_ELSE OP_DUP ${pubKeyLock[6]} OP_EQUAL OP_IF OP_DROP OP_ELSE ${pubKeyLock[7]} OP_EQUALVERIFY OP_ENDIF OP_ENDIF OP_ENDIF OP_ENDIF OP_CHECKSIGVERIFY`);
        //         break;
        // }
        // const lastCode = tbc.Script.fromASM(`OP_RETURN ${tagHex} 32436f6465`);
        const code = tbc.Script.fromString(poolNftCodePre.toString() + " " + firstCode.toString() + " " + lastCode.toString() + " " + poolNftCodeLast.toString());
        return code;
    ...
    getFtlpCodeWithLockTime(poolNftCodeHash: string, address: any, tapeSize: number): tbc.Script {
        const codeHash = poolNftCodeHash;
        const publicKeyHash = tbc.Address.fromString(address).hashBuffer.toString('hex');
        const hash = publicKeyHash + '00';
        const tapeSizeHex = getSize(tapeSize).toString('hex');
