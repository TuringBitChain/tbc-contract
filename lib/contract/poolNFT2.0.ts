...
class poolNFT2 {
    ...
    async swaptoToken_baseTBC(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_tbc: number): Promise<string> {
        ...
        const amount_tbcbn = BigInt(Math.floor(amount_tbc * Math.pow(10, 6)));
        const serviceFee = amount_tbcbn * BigInt(this.service_fee_rate + 5) / BigInt(10000);
        ...
        if (serviceFeeA >= BigInt(42)) {
            tx.to("18uSaPNC2x6SiPvRDrLT99EuPoKv6qd6Q4", Number(serviceFeeA));
        }
        // FTAbyC_Change
    ...
    async swaptoTBC_baseToken(privateKey_from: tbc.PrivateKey, address_to: string, utxo: tbc.Transaction.IUnspentOutput, amount_token: number): Promise<string> {
        ...
        const serviceFee = tbc_amount_decrement * BigInt(this.service_fee_rate + 5) / BigInt(10000);
