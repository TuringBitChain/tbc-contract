...
const TBC20_ARTIFACT_SHA256 = "4efb2bca72f20e2a1e336dbcccc3e81228eb19411ff43cdcb24118115788a186";
...
export interface TBC20Config {
  ...
  symbol?: string;
  /**
   * Exact human-readable SDK/indexer declaration. Canonical SDK transitions
   * preserve these extension bytes, but the current locking script does not
   * authenticate their contents or enforce a consensus supply cap. Number and
   * exponent forms are intentionally rejected.
   */
  supply?: string;
  /** Display precision; defaults to zero when metadata is declared. */
  decimal?: number;
}
...
export interface TBC20AdditionalInput {
   ...
   * view whose scriptSigs are still maximum-size placeholders. Do not depend
   * on tx.hash; the txid is final only after every real script is installed.
   */
  unlock:
    | tbc.Script
    | ((tx: tbc.Transaction, inputIndex: number) => tbc.Script);
  /** Required upper bound when unlock is a callback; static Script inputs infer it. */
...
export interface TBC20BuildResult {
  ...
  txraw: string;
  /**
   * Actual miner fee. It is never below feeForSize(final bytes), but can be
   * higher when a real DER signature is shorter than its 72-byte
...
function scriptFrom(value: TBC20ScriptLike, name: string): tbc.Script {
  ...
  try {
    ...
    fail(`${name} is not a valid script: ${(error as Error).message}`);
  }
}

function bufferFrom(value: string | Buffer, name: string): Buffer {
...
function assertPrivateKey(value: unknown, name: string): asserts value is tbc.PrivateKey {
...
function assertSatoshis(value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0) {
    fail(`${name} must be a non-negative safe integer`);
  }
}

function assertSlotAmount(value: bigint, name: string, allowZero = false): void {
...
function assertPositiveAmount(value: bigint, name: string): void {
  if (typeof value !== "bigint" || value <= 0n) {
    fail(`${name} must be a positive bigint in raw smallest units`);
  }
}

function assertTxid(value: string, name: string): string {
...
function assertVout(value: number, name: string): void {
...
function humanAmountToRaw(value: string, decimal: number, name: string): bigint {
...
function assertMetadataName(value: string): Buffer {
...
function assertMetadataSymbol(value: string): Buffer {
...
function isStrictPushOnly(script: Buffer, start: number): boolean {
  let offset = start;
  while (offset < script.length) {
    ...
    if (dataLength > script.length - offset) return false;
    offset += dataLength;
  }
...
function pushHex(buffer: Buffer): string {
  return new tbc.Script().add(buffer).toHex();
}

function outputScriptHex(utxo: tbc.Transaction.IUnspentOutput): string {
  const script = (utxo as any).script;
...
function outpointKey(utxo: tbc.Transaction.IUnspentOutput): string {
...
function assertCanonicalSigningResult(signature: Buffer, inputIndex: number): void {
  ...
  const parsed: any = tbc.crypto.Signature.fromTxFormat(signature);
...
function buildEstimatedAdditionalUnlock(input: TBC20AdditionalInput): tbc.Script {
  ...
  return tbc.Script.fromBuffer(Buffer.alloc(input.maxUnlockScriptBytes as number));
}

interface InputOutputSnapshot {
  reference: unknown;
  satoshis: number;
  scriptHex: string;
}

function snapshotInputOutputs(tx: tbc.Transaction): InputOutputSnapshot[] {
  return tx.inputs.map((input, inputIndex) => {
    const output = (input as any).output;
...
function inputOutputsEqual(tx: tbc.Transaction, snapshots: readonly InputOutputSnapshot[]): boolean {
...
function cloneForAdditionalCallback(tx: tbc.Transaction): tbc.Transaction {
  ...
  tx.inputs.forEach((input, inputIndex) => {
    ...
    (clone.inputs[inputIndex] as any).output = new tbc.Transaction.Output({
...
function buildAdditionalUnlockOnce(
  ...
  const internals = callbackTx as any;
  ...
  if (script.toBuffer().length > (input.maxUnlockScriptBytes as number)) {
...
function outputAmountSum(tx: tbc.Transaction): number {
  return tx.outputs.reduce((sum, output, index) => {
    ...
    if (!Number.isSafeInteger(next)) fail("aggregate output satoshis exceed JavaScript safe integer range");
    return next;
  }, 0);
}

function inputAmountSum(utxos: readonly tbc.Transaction.IUnspentOutput[]): number {
  return utxos.reduce((sum, utxo, index) => {
    assertSatoshis(utxo.satoshis, `inputs[${index}].satoshis`);
    const next = sum + utxo.satoshis;
    if (!Number.isSafeInteger(next)) fail("aggregate input satoshis exceed JavaScript safe integer range");
    return next;
  }, 0);
}
...
function feeForSize(sizeBytes: number): number {
...
function buildPositionalTokenInputs(
  privateKey: tbc.PrivateKey,
  utxos: readonly tbc.Transaction.IUnspentOutput[],
...
export class TBC20 {
  ...
  static rawToHuman(amountRaw: bigint, decimal: number): string {
  ...
  static feeForSize(sizeBytes: number): number {
    return feeForSize(sizeBytes);
  }

  static addressController(address: string): Buffer {
    if (typeof address !== "string" || address.trim() === "") {
      fail("address is required");
    }
    try {
      const parsed = tbc.Address.fromString(address);
      if ((parsed as any).type !== (tbc.Address as any).PayToPublicKeyHash) {
      ...
      fail(`invalid address: ${(error as Error).message}`);
    }
  }
  ...
  static validateCode(codeScript: TBC20ScriptLike, tapeSize?: number): void {
    ...
    for (const segment of TBC20_TEMPLATE_SEGMENTS) {
      if (segment === "<self.OriginalUTXO36>") {
        ...
        if (code[offset] !== 1) fail("codeScript ConstTapeSize must use a direct one-byte push");
        const currentTapeSize = code[offset + 1];
        if (embeddedTapeSize === undefined) embeddedTapeSize = currentTapeSize;
        if (embeddedTapeSize !== currentTapeSize) fail("codeScript contains inconsistent ConstTapeSize values");
        offset += 2;
      } else if (segment === "<self.Controller21>") {
        if (code[offset] !== 21) fail("codeScript Controller must use a direct 21-byte push");
        offset += 22;
      } else {
    ...
    if (offset !== code.length) fail("codeScript has trailing bytes outside the compiler artifact");
    if (embeddedTapeSize === undefined) fail("codeScript has no embedded ConstTapeSize");
  ...
  mint(
    ...
    const fundingUTXO = typeof amountOrFundingUTXO === "bigint"
      ? fundingUTXOOrOptions as tbc.Transaction.IUnspentOutput
      : amountOrFundingUTXO;
    ...
    const buildSource = (sourceValue: number, estimated: boolean): tbc.Transaction => {
    ...
    const sourceUTXO: tbc.Transaction.IUnspentOutput = {
      txId: source.hash,
      outputIndex: 0,
  ...
  transfer(
    ...
    if (optionsOrPrivateKey instanceof tbc.PrivateKey) {
      ...
      const inputs = buildPositionalTokenInputs(
        optionsOrPrivateKey,
        tokenUTXOs,
        parentTxs,
        ancestorResolvers,
        positionalOptions.contractControllers,
        "positional transfer",
      );
      options = {
        inputs,
    ...
    const outputPlans: Array<{ controller: Buffer; amount: bigint; slots: bigint[] }> = [];
    ...
    receivers.forEach((receiver, receiverIndex) => {
      if (!receiver || typeof receiver !== "object") fail(`receivers[${receiverIndex}] is required`);
    ...
    const buildTransaction = (
      ...
      if (!estimated) {
        ...
        finalScripts.forEach((script, inputIndex) => {
          transaction.setInputScript(
            inputIndex,
            tbc.Script.fromBuffer(Buffer.from(script.toBuffer())),
          );
        });
        const internals = transaction as any;
