...
const getFTVersion = (codeScript, isCoin) => {
    const baseVersion = codeScript.length / 2 === ft_v2_length || isCoin ? 2 : 1;
    if (baseVersion !== 2)
        return 1;
...
class poolNFT2 {
    ...
    async createPoolNFT(privateKey_from, utxo, tag, serviceFeeRate, lpPlan, withLockTime) {
        ...
        this.ft_lp_partialhash = partial_sha256.calculate_partial_hash(ftlpCode.toBuffer().subarray(0, offset));
        this.ft_a_partialhash = partial_sha256.calculate_partial_hash(Buffer.from(FTA.codeScript, "hex").subarray(0, offset));
    ...
    getFtlpCode(poolNftCodeHash, address, tapeSize, isCoin, ftVersion) {
        ...
        const ftlpCodePre = new tbc.Script(ftVersion === 3
            ? ftlpCodePreTemplate
            : toLegacyFtlpCodePre(ftlpCodePreTemplate));
        let ftlpCodeLast;
        if (isCoin) {
            const paddingSize = ftVersion === 3 ? 0x0210 : 0x0241;
            ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA2 0x${paddingSize.toString(16).padStart(4, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
        }
        else {
            if (ftVersion === 2 || ftVersion === 3) {
                const paddingSize = ftVersion === 3 ? 0x0190 : 0x01c1;
                ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA2 0x${paddingSize.toString(16).padStart(4, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
            }
            else {
                const paddingSize = 0x82;
                ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA1 0x${paddingSize.toString(16).padStart(2, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
            }
        }
    ...
    getFtlpCodeWithLockTime(poolNftCodeHash, address, tapeSize, isCoin, ftVersion) {
        ...
        let ftlpCodeLast;
        if (isCoin) {
            const paddingSize = ftVersion === 3 ? 0x01f8 : 0x0229;
            ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA2 0x${paddingSize.toString(16).padStart(4, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
        }
        else {
            if (ftVersion === 2 || ftVersion === 3) {
                const paddingSize = ftVersion === 3 ? 0x0178 : 0x01a9;
                ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA2 0x${paddingSize.toString(16).padStart(4, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
            }
            else {
                const paddingSize = 0x6a;
                ftlpCodeLast = tbc.Script.fromString(`OP_PUSHDATA1 0x${paddingSize.toString(16).padStart(2, "0")} 0x${"ff".repeat(paddingSize)} OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
            }
        }
        const ftlpCode = ftlpCodePre.add(ftlpCodeLast);
