...
const getFTVersion = (codeScript, isCoin) => {
    const codeLength = codeScript.length / 2;
    const baseVersion = codeLength === ft_v2_length || codeLength === coin_length || isCoin ? 2 : 1;
    if (baseVersion !== 2)
        return 1;
    const fillCharLength = (0, util_1.fillCharLengthInFT)(codeScript);
    if (!isCoin && codeLength === coin_length && fillCharLength === 28)
        return 4;
    return fillCharLength === 1 || fillCharLength === 2 ? 3 : 2;
};
const getFTPartialOffset = (codeScript) => {
...
class poolNFT2 {
    ...
    async createPoolNFT(privateKey_from, utxo, tag, serviceFeeRate, lpPlan, withLockTime) {
        ...
        const ftlpOffset = getFTPartialOffset(ftlpCode.toBuffer().toString("hex"));
        const ftaOffset = getFTPartialOffset(FTA.codeScript);
    ...
    getPoolNftCode(txid, vout, lpPlan, ftVersion, tag, isCoin) {
        ...
        const ftCodeSize = isCoin || ftVersion === 4 ? "dc07" : ftVersion === 1 ? "1c06" : "5c07";
