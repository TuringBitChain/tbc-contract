...
class poolNFT2 {
    ...
    async createPoolNFT(privateKey_from, utxo, tag, serviceFeeRate, lpPlan, withLockTime) {
        ...
        this.poolnft_code = this.getPoolNftCode(txSource.hash, 0, lpPlan || 1, ftVersion, tag, isCoin)
            .toBuffer()
            .toString("hex");
    ...
    async createPoolNftWithLock(privateKey_from, utxo, tag, lpCostAddress, lpCostTBC, pubKeyLock, serviceFeeRate, lpPlan, withLockTime) {
        ...
        const isCoin = (0, util_1.isCoinCodeScript)(FTA.codeScript);
        const ftVersion = getFTVersion(FTA.codeScript, isCoin);
        this.poolnft_code = this.getPoolNftCodeWithLock(txSource.hash, 0, lpPlan || 1, lpCostAddress, lpCostTBC, pubKeyLock, ftVersion, tag, isCoin)
            .toBuffer()
            .toString("hex");
        const ftlpCode = (withLockTime ?? false)
            ? this.getFtlpCodeWithLockTime(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), privateKey.toAddress().toString(), FTA.tapeScript.length / 2, isCoin, ftVersion)
