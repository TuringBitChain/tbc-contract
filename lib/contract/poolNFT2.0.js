...
class poolNFT2 {
    ...
    async createPoolNFT(privateKey_from, utxo, tag, serviceFeeRate, lpPlan, withLockTime) {
        ...
        const ftlpCode = withLockTime ?? false
            ? this.getFtlpCodeWithLockTime(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), privateKey.toAddress().toString(), FTA.tapeScript.length / 2, ftVersion)
            : this.getFtlpCode(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), privateKey.toAddress().toString(), FTA.tapeScript.length / 2, ftVersion);
    ...
    async createPoolNftWithLock(privateKey_from, utxo, tag, lpCostAddress, lpCostTBC, pubKeyLock, serviceFeeRate, lpPlan, withLockTime) {
        ...
        FTA.initialize(FTAInfo);
        const ftVersion = FTA.codeScript.length / 2 === ft_v2_length ? 2 : 1;
        this.poolnft_code = this.getPoolNftCodeWithLock(txSource.hash, 0, lpPlan || 1, lpCostAddress, lpCostTBC, pubKeyLock, ftVersion, tag)
            .toBuffer()
            .toString("hex");
        const ftlpCode = withLockTime ?? false
            ? this.getFtlpCodeWithLockTime(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), privateKey.toAddress().toString(), FTA.tapeScript.length / 2, ftVersion)
    ...
    async initPoolNFT(privateKey_from, address_to, utxo, tbc_amount, ft_a, lock_time) {
        ...
        if (this.with_lock_time) {
            ...
            ftlpCodeScript = this.getFtlpCodeWithLockTime(poolnft_codehash.toString("hex"), address_to, tapeSize, ftVersion);
        }
        else {
            ...
            const tapeSize = ftlpTapeScript.toBuffer().length;
            ftlpCodeScript = this.getFtlpCode(poolnft_codehash.toString("hex"), address_to, tapeSize, ftVersion);
        }
        tx.addOutput(new tbc.Transaction.Output({
        ...
        tx.setInputScript({
            ...
            const unlockingScript = FTA.getFTunlock(privateKey, tx, ftPreTX, ftPrePreTxData, 1, fttxo_a.outputIndex);
            return unlockingScript;
        });
        tx.sign(privateKey);
        await tx.sealAsync();
    ...
    async increaseLP(privateKey_from, address_to, utxo, amount_tbc, lock_time) {
        ...
        let fttxo_a;
        try {
            fttxo_a = await API.fetchFtUTXO(this.ft_a_contractTxid, privateKey.toAddress().toString(), changeData.ft_a_difference, ftutxo_codeScript, this.network);
        }
        catch (error) {
    ...
    async consumeLP(privateKey_from, address_to, utxo, amount_lp, lock_time) {
        ...
        let fttxo_c;
        try {
            fttxo_c = await API.fetchFtUTXOsforPool(this.ft_a_contractTxid, poolnft_codehash160, changeData.ft_a_difference, 3, ftutxo_codeScript, this.network);
        }
        catch (error) {
            const errorMessage = error.message === "Insufficient FTbalance, please merge FT UTXOs"
        ...
        for (let i = 0; i < fttxo_c.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i + 1], ftPrePreTxData[i + 1], contractTX, i + 2, fttxo_c[i].outputIndex, ftVersion);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    async swaptoToken_baseTBC(privateKey_from, address_to, utxo, amount_tbc, lpPlan) {
        ...
        for (let i = 0; i < fttxo_c.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i], ftPrePreTxData[i], contractTX, i + 2, fttxo_c[i].outputIndex, ftVersion);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    async fetchFtlpUTXOList(address) {
        ...
        const ftVersion = FTA.codeScript.length / 2 === ft_v2_length ? 2 : 1;
        const ftlpCode = this.with_lock_time ?? false
            ? this.getFtlpCodeWithLockTime(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), address, FTA.tapeScript.length / 2, ftVersion).toBuffer()
            : this.getFtlpCode(tbc.crypto.Hash.sha256(Buffer.from(this.poolnft_code, "hex")).toString("hex"), address, FTA.tapeScript.length / 2, ftVersion).toBuffer();
    ...
    */
        ...
        try {
            ...
            for (let i = 0; i < ftutxos.length; i++) {
                tx.setInputScript({
                    ...
                    const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i], ftPrePreTxData[i], contractTX, i + 1, ftutxos[i].outputIndex, ftVersion);
                    return unlockingScript;
                });
            }
            tx.sign(privateKey);
    ...
    async consumeLpWithLockTime(privateKey_from, address_to, utxo, amount_lp) {
        ...
        for (let i = 0; i < fttxo_c.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i + 1], ftPrePreTxData[i + 1], contractTX, i + 2, fttxo_c[i].outputIndex);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    getPoolNftCode(txid, vout, lpPlan, ftVersion, tag) {
        const writer = new tbc.encoding.BufferWriter();
    ...
    getPoolNftCodeWithLock(txid, vout, lpPlan, lpCostAddress, lpCostTBC, pubKeyLock, ftVersion, tag) {
    ...
    getFtlpCode(poolNftCodeHash, address, tapeSize, ftVersion) {
    ...
    getFtlpCodeWithLockTime(poolNftCodeHash, address, tapeSize, ftVersion) {
        const codeHash = poolNftCodeHash;
        const publicKeyHash = tbc.Address.fromString(address).hashBuffer.toString("hex");
        const hash = publicKeyHash + "00";
        const tapeSizeHex = (0, poolnftunlock_1.getSize)(tapeSize).toString("hex");
        ...
        if (ftVersion === 2) {
            ...
            ftlpCodeLast = tbc.Script.fromString(`OP_NIP OP_PUSHDATA1 0x6a 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff OP_DROP OP_RETURN 0x15 0x${hash} 0x05 0x02436f6465`);
        }
        const ftlpCode = ftlpCodePre.add(ftlpCodeLast);
        return ftlpCode;
    }
