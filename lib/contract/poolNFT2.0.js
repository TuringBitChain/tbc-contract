    ...
    async swaptoToken_baseTBC(privateKey_from, address_to, utxo, amount_tbc, lpPlan) {
        ...
        for (let i = 0; i < fttxo_c.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i], ftPrePreTxData[i], contractTX, i + 2, fttxo_c[i].outputIndex);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
        ...
        try {
            ...
            for (let i = 0; i < ftutxos.length; i++) {
                tx.setInputScript({
                    ...
                    const unlockingScript = FTA.getFTunlockSwap(privateKey, tx, ftPreTX[i], ftPrePreTxData[i], contractTX, i + 1, ftutxos[i].outputIndex);
                    return unlockingScript;
                });
            }
            tx.sign(privateKey);
