...
class poolNFT {
    ...
    async createPoolNFT(privateKey_from) {
        ...
        // } else if (amount_lp) {
        //     this.ft_lp_amount = BigInt(amount_lp * Math.pow(10, 6));
        // } else {
        //     throw new Error('Invalid Input');
        // }
    ...
    async initPoolNFT(privateKey_from, address_to, tbc_amount, ft_a) {
        const privateKey = privateKey_from;
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
    ...
    async increaseLP(privateKey_from, address_to, amount_tbc) {
        const privateKey = privateKey_from;
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
    ...
    async consumLP(privateKey_from, address_to, amount_lp) {
        const privateKey = privateKey_from;
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
        ...
        ({ amountHex, changeHex } = FT.buildTapeAmount(amount_lpbn, lpTapeAmountSetIn, 1));
    ...
    async swaptoToken(privateKey_from, address_to, amount_token) {
        const privateKey = privateKey_from;
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
    ...
    async swaptoTBC(privateKey_from, address_to, amount_tbc) {
        const privateKey = privateKey_from;
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
    ...
    async mergeFTLP(privateKey_from) {
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
        const privateKey = privateKey_from;
        ...
        try {
            ...
            await poolNFT.broadcastTXraw(txraw, this.network);
            // wait 5 seconds
    ...
    async mergeFTinPool(privateKey_from) {
        const FTA = new FT({ txidOrParams: this.ft_a_contractTxid, network: this.network });
        const FTAInfo = await API.fetchFtInfo(FTA.contractTxid, this.network);
        await FTA.initialize(FTAInfo);
        const privateKey = privateKey_from;
