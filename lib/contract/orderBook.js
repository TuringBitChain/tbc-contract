...
class OrderBook {
    ...
    buildBuyOrderTX(holdAddress, saleVolume, unitPrice, feeRate, ftID, utxos, ftutxos, preTXs) {
        ...
        this.fee_rate = feeRate;
        this.ft_a_contract_id = ftID;
        this.ft_a_contract_partialhash = partial_sha256.calculate_partial_hash(Buffer.from(ftutxos[0].script, "hex").subarray(0, 1536));
        const tx = new tbc.Transaction();
        tx.from(ftutxos);
        tx.from(utxos);
        // Buy Order Output
    ...
    makeBuyOrder_privateKey(privateKey, saleVolume, unitPrice, feeRate, ftID, utxos, ftutxos, preTXs, prepreTxData) {
        ...
        let { amountHex, changeHex } = FT.buildTapeAmount((saleVolume * unitPrice) / this.precision, tapeAmountSetIn);
        const ftCode = ftutxos[0].script;
        const ftTape = preTXs[0].outputs[ftutxos[0].outputIndex + 1].script.toHex();
        const buyOrderHash160 = tbc.crypto.Hash.sha256ripemd160(tbc.crypto.Hash.sha256(buyOrder.toBuffer())).toString("hex");
        const ftCodeBuy = FT.buildFTtransferCode(ftCode, buyOrderHash160);
    ...
    cancelBuyOrder_privateKey(privateKey, buyutxo, buyPreTX, ftutxo, ftPreTX, ftPrePreTxData, utxos) {
        ...
        tx.setInputScript({
            ...
            const unlockingScript = new FT(buyData.ftID).getFTunlockSwap(privateKey, tx, ftPreTX, ftPrePreTxData, buyPreTX, 1, ftutxo.outputIndex);
            return unlockingScript;
        });
        tx.sign(privateKey);
        tx.seal();
    ...
    fillSigsMakeBuyOrder(buyOrderTxRaw, sigs, publicKey, 
    ...
    fillSigsCancelBuyOrder(buyOrderTxRaw, sigs, publicKey, 
    // ftutxo: tbc.Transaction.IUnspentOutput,
    ftPreTX, ftPrePreTxData) {
        const tx = new tbc.Transaction(buyOrderTxRaw);
        tx.setInputScript({
            inputIndex: 0,
        ...
        tx.setInputScript({
            ...
            const unlockingScript = FT.getFTunlock(sigs[1], publicKey, tx, ftPreTX, ftPrePreTxData, 1, tx.inputs[1].outputIndex);
            return unlockingScript;
        });
