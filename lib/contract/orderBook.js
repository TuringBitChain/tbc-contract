...
const getCoinInputLockTime = (context) => {
    ...
    return stableCoin.getLockTimeFromTape(tapeOutput.script);
};
const applyCoinInputLockTimes = (tx, contexts) => {
    ...
    for (const context of contexts) {
        ...
        lockTimeMax = Math.max(lockTimeMax, getCoinInputLockTime(context));
    }
    if (hasCoinInput)
...
const validateCoinInputLockTimes = (tx, contexts) => {
    for (const context of contexts) {
        ...
        if (tx.nLockTime < requiredLockTime) {
...
class OrderBook {
    ...
    buildSellOrderTX(holdAddress, taxAddress, saleVolume, unitPrice, feeRate, ftID, ftCodeScript, utxos) {
        ...
        const isCoin = (0, util_1.isCoinCodeScript)(ftCodeScript);
        const partialOffset = (0, ftscript_1.getFTPartialOffset)(ftCodeScript);
    ...
    buildBuyOrderTX(holdAddress, taxAddress, saleVolume, unitPrice, feeRate, ftID, utxos, ftutxos, preTXs) {
        ...
        const partialOffset = (0, ftscript_1.getFTPartialOffset)(ftutxos[0].script);
    ...
    buildCancelBuyOrderTX(buyutxo, ftutxo, ftPreTX, utxos) {
        ...
        const isCoin = (0, util_1.isCoinCodeScript)(ftutxo.script);
        const tx = new tbc.Transaction();
    ...
    fillSigsMakeBuyOrder(buyOrderTxRaw, sigs, publicKey, preTXs, prepreTxData) {
        ...
        if (!Array.isArray(sigs) || sigs.some((sig) => !(0, util_1._isValidHexString)(sig)))
            throw new Error("Invalid Signatures array");
        if (prepreTxData.some((data) => !(0, util_1._isValidHexString)(data)))
            throw new Error("Invalid PrePreTxData array");
        const tx = new tbc.Transaction(buyOrderTxRaw);
        const isCoin = (0, util_1.isCoinCodeScript)(tx.outputs[1].script.toHex());
        ...
        for (let i = 0; i < preTXs.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = FT.getFTunlock(sigs[i], publicKey, tx, preTXs[i], prepreTxData[i], i, tx.inputs[i].outputIndex, isCoin);
    ...
    fillSigsCancelBuyOrder(buyOrderTxRaw, sigs, publicKey, buyPreTX, ftPreTX, ftPrePreTxData) {
        ...
        if (!Array.isArray(sigs) || sigs.some((sig) => !(0, util_1._isValidHexString)(sig)))
            throw new Error("Invalid Signatures array");
        if (!(0, util_1._isValidHexString)(ftPrePreTxData))
            throw new Error("Invalid FtPrePreTxData string");
        const tx = new tbc.Transaction(buyOrderTxRaw);
        const isCoin = (0, util_1.isCoinCodeScript)(tx.outputs[0].script.toHex());
        const ftVersion = (0, ftscript_1.getFTVersion)(tx.outputs[0].script.toHex(), isCoin);
        ...
        tx.setInputScript({
            ...
            const unlockingScript = FT.getFTunlockSwap(sigs[1], publicKey, tx, ftPreTX, ftPrePreTxData, buyPreTX, 1, tx.inputs[1].outputIndex, ftVersion, isCoin);
            return unlockingScript;
        });
    ...
    matchOrder(privateKey, buyutxo, buyPreTX, ftutxo, ftPreTX, ftPrePreTxData, sellutxo, sellPreTX, utxos, ftFeeAddress, tbcFeeAddress) {
        ...
        const isCoin = (0, util_1.isCoinCodeScript)(ftutxo.script);
        const ftVersion = (0, ftscript_1.getFTVersion)(ftutxo.script, isCoin);
        applyCoinInputLockTimes(tx, [
            {
                inputIndex: 1,
                isCoin,
        ...
        tx.setInputScript({
            ...
            const unlockingScript = new FT(buyData.ftID).getFTunlockSwap(privateKey, tx, ftPreTX, ftPrePreTxData, buyPreTX, 1, ftutxo.outputIndex, ftVersion, isCoin);
            return unlockingScript;
        });
        tx.setInputScript({
            inputIndex: 2,
    ...
    async makeSellOrder_privateKeyOnline(privateKey, taxAddress, saleVolume, unitPrice, feeRate, ftID) {
        ...
        this.fee_rate = feeRate;
        this.ft_a_contract_id = ftID;
        const isCoin = (0, util_1.isCoinCodeScript)(TokenInfo.codeScript);
        const partialOffset = (0, ftscript_1.getFTPartialOffset)(TokenInfo.codeScript);
    ...
    async makeBuyOrder_privateKeyOnline(privateKey, taxAddress, saleVolume, unitPrice, feeRate, ftID) {
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = new FT(ftID).getFTunlock(privateKey, tx, preTXs[i], prepreTxData[i], i, ftutxos[i].outputIndex, isCoin);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    getTokenPartialHash(codeScript) {
        ...
        const partialOffset = (0, ftscript_1.getFTPartialOffset)(codeScript);
    ...
    buildTokenSellOrderTX(holdAddress, taxAddress, saleVolume, unitPrice, feeRate, ftaID, ftbID, ftaCodeScript, ftbCodeScript, utxos, ftutxos, preTXs) {
        ...
        const { amountHex, changeHex } = FT.buildTapeAmount(saleVolume, tapeAmountSetIn);
        const ftCode = ftutxos[0].script;
        const ftTape = preTXs[0].outputs[ftutxos[0].outputIndex + 1].script.toHex();
    ...
    buildTokenBuyOrderTX(holdAddress, taxAddress, saleVolume, unitPrice, feeRate, ftaID, ftbID, ftaCodeScript, ftbCodeScript, utxos, ftutxos, preTXs) {
        ...
        const { amountHex, changeHex } = FT.buildTapeAmount(ftAmount, tapeAmountSetIn);
        const ftCode = ftutxos[0].script;
        const ftTape = preTXs[0].outputs[ftutxos[0].outputIndex + 1].script.toHex();
        const buyOrderHash160 = tbc.crypto.Hash.sha256ripemd160(tbc.crypto.Hash.sha256(buyOrder.toBuffer())).toString("hex");
        tx.addOutput(new tbc.Transaction.Output({
    ...
    fillSigsMakeTokenOrderTX(tokenOrderTxRaw, sigs, publicKey, preTXs, prepreTxData) {
        ...
        for (let i = 0; i < preTXs.length; i++) {
            tx.setInputScript({
                inputIndex: i,
            }, (tx) => FT.getFTunlock(sigs[i], publicKey, tx, preTXs[i], prepreTxData[i], i, tx.inputs[i].outputIndex, isCoin));
        }
        for (let i = preTXs.length; i < tx.inputs.length; i++) {
            tx.setInputScript({
                inputIndex: i,
    ...
    fillSigsCancelTokenOrderTX(tokenOrderTxRaw, sigs, publicKey, tokenOrderPreTX, ftPreTX, ftPrePreTxData) {
        ...
        tx.setInputScript({
            inputIndex: 1,
        }, (tx) => FT.getFTunlockSwap(sigs[1], publicKey, tx, ftPreTX, ftPrePreTxData, tokenOrderPreTX, 1, tx.inputs[1].outputIndex, ftVersion, isCoin));
        for (let i = 2; i < tx.inputs.length; i++) {
            tx.setInputScript({
                inputIndex: i,
            }, tbc.Script.fromASM(`${sigs[i]} ${publicKey}`));
    ...
    buildMatchTokenOrderTransaction(buyutxo, buyPreTX, buyFtUtxo, buyFtPreTX, sellutxo, sellPreTX, sellFtUtxo, sellFtPreTX, utxos, ftaFeeAddress, ftbFeeAddress) {
        ...
        applyCoinInputLockTimes(tx, [
            {
                inputIndex: 1,
                isCoin: (0, util_1.isCoinCodeScript)(buyFtUtxo.script),
                preTX: buyFtPreTX,
                preTxVout: buyFtUtxo.outputIndex,
            },
            {
                inputIndex: 3,
                isCoin: (0, util_1.isCoinCodeScript)(sellFtUtxo.script),
                preTX: sellFtPreTX,
        ...
        const { amountHex: tokenABuyerAmountHex } = FT.buildTapeAmount(tokenABuyerAmount, [tokenABalance], 3);
        const { amountHex: tokenATaxAmountHex, changeHex: tokenAChangeHex, } = FT.buildTapeAmount(tokenATaxAmount, [tokenABalance - tokenABuyerAmount], 3);
        const { amountHex: tokenBSellerAmountHex } = FT.buildTapeAmount(tokenBSellerAmount, [tokenBBalance], 1);
        const { amountHex: tokenBTaxAmountHex, changeHex: tokenBChangeHex, } = FT.buildTapeAmount(tokenBTaxAmount, [tokenBBalance - tokenBSellerAmount], 1);
    ...
    matchTokenOrder(privateKey, buyutxo, buyPreTX, buyFtUtxo, buyFtPreTX, buyFtPrePreTxData, sellutxo, sellPreTX, sellFtUtxo, sellFtPreTX, sellFtPrePreTxData, utxos, ftaFeeAddress, ftbFeeAddress) {
        if (!(0, util_1._isValidHexString)(buyFtPrePreTxData))
            throw new Error("Invalid BuyFtPrePreTxData string");
        if (!(0, util_1._isValidHexString)(sellFtPrePreTxData))
        ...
        const buyFtIsCoin = (0, util_1.isCoinCodeScript)(buyFtUtxo.script);
        const buyFtVersion = (0, ftscript_1.getFTVersion)(buyFtUtxo.script, buyFtIsCoin);
        tx.setInputScript({
            inputIndex: 1,
        }, (tx) => new FT(buyData.ftbID).getFTunlockSwap(privateKey, tx, buyFtPreTX, buyFtPrePreTxData, buyPreTX, 1, buyFtUtxo.outputIndex, buyFtVersion, buyFtIsCoin, true));
        tx.setInputScript({
            inputIndex: 2,
        }, (tx) => this.getTokenOrderUnlock(tx, sellPreTX, sellutxo.outputIndex));
        const sellFtIsCoin = (0, util_1.isCoinCodeScript)(sellFtUtxo.script);
        const sellFtVersion = (0, ftscript_1.getFTVersion)(sellFtUtxo.script, sellFtIsCoin);
        tx.setInputScript({
            inputIndex: 3,
        }, (tx) => new FT(sellData.ftaID).getFTunlockSwap(privateKey, tx, sellFtPreTX, sellFtPrePreTxData, sellPreTX, 3, sellFtUtxo.outputIndex, sellFtVersion, sellFtIsCoin, true));
        tx.sign(privateKey);
        tx.seal();
        return tx.uncheckedSerialize();
    }
    async makeTokenSellOrder_privateKeyOnline(privateKey, taxAddress, saleVolume, unitPrice, feeRate, ftaID, ftbID) {
        ...
        const ftautxo_codeScript = FT.buildFTtransferCode(TokenInfoA.codeScript, privateKey.toAddress().toString())
            .toBuffer()
            .toString("hex");
        const isCoinA = (0, util_1.isCoinCodeScript)(TokenInfoA.codeScript);
        const partialOffsetA = (0, ftscript_1.getFTPartialOffset)(TokenInfoA.codeScript);
        ...
        const isCoinB = (0, util_1.isCoinCodeScript)(TokenInfoB.codeScript);
        const partialOffsetB = (0, ftscript_1.getFTPartialOffset)(TokenInfoB.codeScript);
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = new FT(ftaID).getFTunlock(privateKey, tx, preTXs[i], prepreTxData[i], i, ftutxos[i].outputIndex, isCoinA);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    async cancelTokenSellOrder_privateKeyOnline(privateKey, sellutxo) {
        ...
        tx.setInputScript({
            ...
            const unlockingScript = new FT(sellData.ftaID).getFTunlockSwap(privateKey, tx, ftPreTX, ftPrePreTxData, sellPreTX, 1, ftutxo.outputIndex, ftVersion, isCoin);
            return unlockingScript;
        });
        tx.sign(privateKey);
        tx.seal();
    ...
    async makeTokenBuyOrder_privateKeyOnline(privateKey, taxAddress, saleVolume, unitPrice, feeRate, ftaID, ftbID) {
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = new FT(ftbID).getFTunlock(privateKey, tx, preTXs[i], prepreTxData[i], i, ftutxos[i].outputIndex, isCoinB);
                return unlockingScript;
            });
        }
        tx.sign(privateKey);
    ...
    async cancelTokenBuyOrder_privateKeyOnline(privateKey, buyutxo) {
        ...
        tx.setInputScript({
            ...
            const unlockingScript = new FT(buyData.ftbID).getFTunlockSwap(privateKey, tx, ftPreTX, ftPrePreTxData, buyPreTX, 1, ftutxo.outputIndex, ftVersion, isCoin);
            return unlockingScript;
        });
        tx.sign(privateKey);
        tx.seal();
    ...
    async matchTokenOrderOnline(privateKey, buyutxo, sellutxo, ftaFeeAddress, ftbFeeAddress) {
        ...
        const buyFtPrePreTxData = await API.fetchFtPrePreTxData(buyPreTX, buyFtUtxo.outputIndex, network);
        const sellFtPrePreTxData = await API.fetchFtPrePreTxData(sellPreTX, sellFtUtxo.outputIndex, network);
        const utxos = [await API.fetchUTXO(privateKey, utxoFee, network)];
        const buyData = OrderBook.getTokenOrderData(buyutxo.script);
        const sellData = OrderBook.getTokenOrderData(sellutxo.script);
        if (buyData.ftaID !== sellData.ftaID || buyData.ftbID !== sellData.ftbID)
        ...
        tx.setInputScript({
            inputIndex: 1,
        }, (tx) => new FT(buyData.ftbID).getFTunlockSwap(privateKey, tx, buyPreTX, buyFtPrePreTxData, buyPreTX, 1, buyFtUtxo.outputIndex, buyFtVersion, buyFtIsCoin, true));
        tx.setInputScript({
            inputIndex: 2,
        }, (tx) => this.getTokenOrderUnlock(tx, sellPreTX, sellutxo.outputIndex));
        const sellFtIsCoin = (0, util_1.isCoinCodeScript)(sellFtUtxo.script);
        ...
        tx.setInputScript({
            inputIndex: 3,
        }, (tx) => new FT(sellData.ftaID).getFTunlockSwap(privateKey, tx, sellPreTX, sellFtPrePreTxData, sellPreTX, 3, sellFtUtxo.outputIndex, sellFtVersion, sellFtIsCoin, true));
        tx.sign(privateKey);
        tx.seal();
        return tx.uncheckedSerialize();
    }
