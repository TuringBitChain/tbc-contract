...
var MultiSig = /** @class */ (function () {
  ...
  */
    ...
    MultiSig.p2pkhToMultiSig_sendTBC = function (address_from, address_to, amount_tbc, utxos, privateKey) {
        ...
        var amount_satoshis = Math.ceil(amount_tbc * Math.pow(10, 6));
        var tx = new tbc.Transaction()
            .from(utxos)
     ...
     * @returns The raw serialized transaction string
     */
    MultiSig.p2pkhToMultiSig_transferFT = function (address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, privateKey) {
        ...
        var amountbn = BigInt(Math.ceil(ft_amount * Math.pow(10, decimal)));
        ...
        var maxAmount = Math.ceil(Math.pow(10, 18 - decimal));
        ...
        var tx = new tbc.Transaction()
            .from(ftutxos)
            .from(utxo);
        var hash = tbc.crypto.Hash.sha256ripemd160(tbc.crypto.Hash.sha256(tbc.Script.fromASM(MultiSig.getMultiSigLockScript(address_to)).toBuffer())).toString("hex");
    ...
    MultiSig.buildMultiSigTransaction_transferFT = function (address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, contractTX, privateKey) {
        ...
        var script_asm = MultiSig.getMultiSigLockScript(address_from);
        var hash_from = tbc.crypto.Hash.sha256ripemd160(tbc.crypto.Hash.sha256(tbc.Script.fromASM(script_asm).toBuffer())).toString("hex");
        var amountbn = BigInt(Math.ceil(ft_amount * Math.pow(10, decimal)));
        var tapeAmountSum = BigInt(0);
        for (var i = 0; i < ftutxos.length; i++) {
            tapeAmountSetIn.push(ftutxos[i].ftBalance);
     ...
     * @returns An array of signatures
     */
    MultiSig.signMultiSigTransaction_transferFT = function (address_from, ft, multiSigTxraw, privateKey) {
        var script_asm = MultiSig.getMultiSigLockScript(address_from);
        var hash_from = tbc.crypto.Hash.sha256ripemd160(tbc.crypto.Hash.sha256(tbc.Script.fromASM(script_asm).toBuffer())).toString("hex");
        var ftutxo_codeScript = FT.buildFTtransferCode(ft.codeScript, hash_from).toBuffer().toString('hex');
        var txraw = multiSigTxraw.txraw, amounts = multiSigTxraw.amounts;
        var tx = new tbc.Transaction(txraw);
        tx.inputs[0].output = new tbc.Transaction.Output({ script: tbc.Script.fromASM(script_asm), satoshis: amounts[0] });
        for (var i = 1; i < tx.inputs.length; i++) {
