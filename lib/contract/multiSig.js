...
const applyStableCoinInputLockTimes = (tx, ftutxos, preTXs, inputIndexOffset) => {
    if (ftutxos.length === 0)
        return false;
    const isCoin = (0, ftscript_1.isCoinCodeScript)(ftutxos[0].script);
    if (ftutxos.some((ftutxo) => (0, ftscript_1.isCoinCodeScript)(ftutxo.script) !== isCoin)) {
    ...
    for (let i = 0; i < ftutxos.length; i++) {
        ...
        lockTimeMax = Math.max(lockTimeMax, stableCoin.getLockTimeFromTape(tapeOutput.script));
    }
...
class MultiSig {
    ...
    static createMultiSigWallet(address_from, pubKeys, signatureCount, publicKeyCount, tbc_amount, utxos, privateKey) {
        ...
        tx.addOutput(new tbc.Transaction.Output({
            ...
            satoshis: Number((0, util_1.parseDecimalToBigInt)(tbc_amount, 6)),
        }));
     ...
     * @returns The raw serialized transaction string
     */
    static p2pkhToMultiSig_transferFT(address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, privateKey, tbc_amount) {
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = ft.getFTunlock(privateKey, tx, preTXs[i], prepreTxDatas[i], i, ftutxos[i].outputIndex, isCoin);
                return unlockingScript;
            });
        }
        tx.sign(privateKey).seal();
    ...
    static buildMultiSigTransaction_transferFT(address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, contractTX, privateKey) {
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = ft.getFTunlockSwap(privateKey, tx_source, preTXs[i], prepreTxDatas[i], contractTX, i + 1, ftutxos[i].outputIndex, ftVersion, isCoin);
                return unlockingScript;
            });
        }
        const txraw = tx.uncheckedSerialize();
    ...
    static validateMultiSigAddress(address) {
        ...
        if (signatureCount < 1 ||
            ...
            publicKeyCount < 3 ||
            publicKeyCount > 10 ||
            signatureCount > publicKeyCount) {
            return false;
        }
        const payload = buf.subarray(0, -4);
        const checksum = buf.subarray(-4);
        const hash = tbc.crypto.Hash.sha256sha256(payload);
