...
var MultiSig = /** @class */ (function () {
     ...
     */
    MultiSig.createMultiSigWallet = function (address_from, pubKeys, signatureCount, publicKeyCount, tbc_amount, utxos, privateKey) {
     ...
     * @returns The raw serialized transaction string
     */
    MultiSig.buildMultiSigTransaction_sendTBCToP2pkh = function (address_from, address_to, amount_tbc, utxos) {
     ...
     * @returns The raw serialized transaction string
     */
    MultiSig.buildMultiSigTransaction_sendTBCToMultiSig = function (address_from, address_to, amount_tbc, utxos) {
        var script_asm_from = MultiSig.getMultiSigLockScript(address_from);
        ...
        if (count - amount_satoshis - 1100 > 80) {
            ...
            return [
                { txraw: tx_source_raw, amounts: amounts },
                { txraw: tx_raw, amounts: [count - amount_satoshis - 1000] },
            ];
        }
        else {
            return [{ txraw: tx_source_raw, amounts: amounts }];
        }
    };
    /**
   ...
   * @returns An array of signatures
   */
    MultiSig.batchSignMultiSigTransaction_sendTBC = function (address_from, multiSigTxraws, privateKey) {
        var allSigs = [];
     ...
     * @returns The raw serialized transaction string
     */
    MultiSig.batchFinishMultiSigTransaction_sendTBC = function (txraws, sigs, pubKeys) {
        var finishedTxs = [];
     ...
     * @returns The raw serialized transaction string
     */
    MultiSig.buildMultiSigTransaction_transferFT = function (address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, contractTX, privateKey) {
        var code = ft.codeScript;
        var tape = ft.tapeScript;
        var decimal = ft.decimal;
        var tapeAmountSetIn = [];
        ...
        var _loop_3 = function (i) {
            tx.setInputScript({
                ...
                var unlockingScript = ft.getFTunlockSwap(privateKey, tx_source, preTXs[i], prepreTxDatas[i], contractTX, i + 1, ftutxos[i].outputIndex);
                return unlockingScript;
            });
        };
        for (var i = 0; i < ftutxos.length; i++) {
     ...
     * @returns An array of signatures
     */
    MultiSig.batchSignMultiSigTransaction_transferFT = function (multiSig_address, multiSigTxraws, privateKey) {
        var allSigs = [];
        for (var txIndex = 0; txIndex < multiSigTxraws.length; txIndex++) {
     ...
     */
    MultiSig.batchFinishMultiSigTransaction_transferFT = function (txraws, sigs, pubKeys) {
        var finishedTxs = [];
        for (var txIndex = 0; txIndex < txraws.length; txIndex++) {
