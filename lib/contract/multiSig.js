...
const getFTVersion = (codeScript) => {
    const codeLength = codeScript.length / 2;
    const baseVersion = codeLength === ft_v2_length || codeLength === coin_length ? 2 : 1;
    if (baseVersion !== 2)
        return 1;
    const fillCharLength = (0, util_1.fillCharLengthInFT)(codeScript);
    if (codeLength === coin_length && fillCharLength === 28)
        return 4;
    ...
    static buildMultiSigTransaction_transferFT(address_from, address_to, ft, ft_amount, utxo, ftutxos, preTXs, prepreTxDatas, contractTX, privateKey) {
        ...
        for (let i = 0; i < ftutxos.length; i++) {
            tx.setInputScript({
                ...
                const unlockingScript = ft.getFTunlockSwap(privateKey, tx_source, preTXs[i], prepreTxDatas[i], contractTX, i + 1, ftutxos[i].outputIndex, ftVersion);
                return unlockingScript;
            });
        }
        const txraw = tx.uncheckedSerialize();
