...
const contractToken_1 = require("../util/common/contractToken");
const coinTbc20Code_1 = require("../util/coin/coinTbc20Code");
const stableCoin = require("./stableCoin");
const SUPPORTED_FT_CODE_LENGTHS = [
    ftscript_1.FT_V2_CODE_LENGTH,
    ftscript_1.LEGACY_COIN_CODE_LENGTH,
    ftscript_1.FT_V4_CODE_LENGTH,
    ...contractToken_1.modernCodeOffsets.keys(),
];
const validateFTCodeLength = (codeLength) => {
    if (!SUPPORTED_FT_CODE_LENGTHS.includes(codeLength)) {
        throw new Error(`Unsupported FT code length ${codeLength}; expected ${SUPPORTED_FT_CODE_LENGTHS.join(", ")}`);
    }
};
const tokenLockTime = (code, tape) => (0, contractToken_1.tokenKind)(code) === "coinTbc20"
    ? coinTbc20Code_1.CoinTBC20.parseTape(tape).lockTime
    : stableCoin.getLockTimeFromTape(tape);
const mergeLockTimes = (a, b) => {
    if (a && b && (a < 500000000) !== (b < 500000000))
        throw new Error("HTLC: cannot combine height and timestamp locks");
    return Math.max(a, b);
};
const tokenUnlockEstimate = (code) => (0, contractToken_1.tokenKind)(code) === "legacy" ? 2000 : 12000;
// Authenticate the Code/Tape pair before allocating balances or binding it to HTLC.
const validateTokenInputs = (utxos, parents) => {
    const identity = Buffer.from(utxos[0].script, "hex").subarray(0, (0, contractToken_1.getFTPartialOffset)(utxos[0].script));
    utxos.forEach((utxo, i) => {
        ...
        const prefix = Buffer.from(utxo.script, "hex").subarray(0, (0, contractToken_1.getFTPartialOffset)(utxo.script));
        if (!prefix.equals(identity))
            throw new Error("HTLC: token inputs must have the same identity");
        if (BigInt(utxo.ftBalance) !== (0, util_1.getFtBalanceFromTape)(tape.script.toHex()))
...
function deployHTLCToken(sender, receiver, hashlock, timelock, ftAmount, ftutxos, utxo, preTX, prepreTxData) {
    ...
    const isCoin = (0, contractToken_1.isCoinCodeScript)(ftutxos[0].script);
    ...
    if (isCoin) {
        for (let i = 0; i < ftutxos.length; i++) {
            lockTimeMax = mergeLockTimes(lockTimeMax, tokenLockTime(ftutxos[i].script, preTX[i].outputs[ftutxos[i].outputIndex + 1].script));
        }
    }
    const { amountHex, changeHex } = contractToken_1.ContractToken.buildTapeAmount(amountbn, tapeAmountSetIn);
    ...
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferCode(ftCodeTemplate, htlcHash160),
        satoshis: 500,
    }));
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferTape(ftTapeTemplate, amountHex),
        satoshis: 0,
    }));
    if (amountbn < tapeAmountSum) {
        tx.addOutput(new tbc.Transaction.Output({
            script: contractToken_1.ContractToken.buildFTtransferCode(ftCodeTemplate, sender),
            satoshis: 500,
        }));
        tx.addOutput(new tbc.Transaction.Output({
            script: contractToken_1.ContractToken.buildFTtransferTape(ftTapeTemplate, changeHex),
            satoshis: 0,
        }));
    }
...
function fillSigDeployHTLCToken(deployRaw, sigs, publicKey, preTX, prepreTxData) {
    ...
    const ftCodeScript = preTX[0].outputs[tx.inputs[0].outputIndex].script.toHex();
    const isCoin = (0, contractToken_1.isCoinCodeScript)(ftCodeScript);
    for (let i = 0; i < ftInputCount; i++) {
        tx.setInputScript({ inputIndex: i }, (currentTX) => {
            return contractToken_1.ContractToken.getFTunlock(sigs[i], publicKey, currentTX, preTX[i], prepreTxData[i], i, tx.inputs[i].outputIndex, isCoin);
        });
    }
...
function withdrawHTLCToken(receiver, htlcutxo, ftutxo, deployTX, utxo) {
    ...
    const isCoin = (0, contractToken_1.isCoinCodeScript)(ftutxo.script);
    validateFTCodeLength(ftCodeLen);
    ...
    const { amountHex } = contractToken_1.ContractToken.buildTapeAmount(totalAmount, [totalAmount], 1);
    const tx = new tbc.Transaction().from(htlcutxo).from(ftutxo).from(utxo);
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferCode(ftutxo.script, receiver),
        satoshis: 500,
    }));
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferTape(ftTapeTemplate, amountHex),
...
function fillSigWithdrawHTLCToken(withdrawRaw, sigs, publicKey, secret, deployTX, prepreTxData) {
    ...
    if (!(0, contractToken_1.isTokenProof)(prepreTxData)) {
    ...
    const isCoin = (0, contractToken_1.isCoinCodeScript)(ftCodeScript);
    const ftVersion = (0, contractToken_1.getFTVersion)(ftCodeScript, isCoin);
    ...
    tx.setInputScript({ inputIndex: 1 }, (currentTX) => {
        return contractToken_1.ContractToken.getFTunlockSwap(sigs[1], publicKey, currentTX, deployTX, prepreTxData, deployTX, 1, ftCodeOutputIndex, ftVersion, isCoin);
    });
    // [2] TBC fee P2PKH
...
function refundHTLCToken(sender, htlcutxo, ftutxo, deployTX, utxo, timelock) {
    ...
    const tx = new tbc.Transaction().from(htlcutxo).from(ftutxo).from(utxo);
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferCode(ftutxo.script, sender),
        satoshis: 500,
    }));
    tx.addOutput(new tbc.Transaction.Output({
        script: contractToken_1.ContractToken.buildFTtransferTape(ftTapeTemplate, amountHex),
...
function deployHTLCTokenWithSign(sender, receiver, hashlock, timelock, ftAmount, ftutxos, utxo, preTX, prepreTxData, privateKey) {
    ...
    for (let i = 0; i < ftutxos.length; i++) {
        tx.setInputScript({ inputIndex: i }, (currentTX) => {
            ...
            return contractToken_1.ContractToken.getFTunlock(sig, publicKey, currentTX, preTX[i], prepreTxData[i], i, ftutxos[i].outputIndex, isCoin);
        });
    }
...
function withdrawHTLCTokenWithSign(privateKey, receiver, htlcutxo, ftutxo, deployTX, prepreTxData, utxo, secret) {
    ...
    tx.setInputScript({ inputIndex: 1 }, (currentTX) => {
        ...
        return contractToken_1.ContractToken.getFTunlockSwap(sig, publicKey, currentTX, deployTX, prepreTxData, deployTX, 1, ftutxo.outputIndex, ftVersion, isCoin);
    });
    tx.sign(privateKeyObj);
    tx.seal();
