...
function fillSigWithdrawHTLCToken(withdrawRaw, sigs, publicKey, secret, deployTX, prepreTxData) {
    ...
    const isCoin = (0, util_1.isCoinCodeScript)(ftCodeScript);
    const ftVersion = (0, ftscript_1.getFTVersion)(ftCodeScript, isCoin);
    ...
    tx.setInputScript({ inputIndex: 1 }, (currentTX) => {
        return FT.getFTunlockSwap(sigs[1], publicKey, currentTX, deployTX, prepreTxData, deployTX, 1, ftCodeOutputIndex, ftVersion, isCoin);
    });
    // [2] TBC fee P2PKH
...
function withdrawHTLCTokenWithSign(privateKey, receiver, htlcutxo, ftutxo, deployTX, prepreTxData, utxo, secret) {
    ...
    tx.setInputScript({ inputIndex: 1 }, (currentTX) => {
        ...
        return FT.getFTunlockSwap(sig, publicKey, currentTX, deployTX, prepreTxData, deployTX, 1, ftutxo.outputIndex, ftVersion, isCoin);
    });
    tx.sign(privateKeyObj);
    tx.seal();
