...
const coinTbc20unlock_1 = require("../util/coin/coinTbc20unlock");
...
function fail(message) { throw new Error(`Coin: ${message}`); }
...
function utxoFrom(tx, vout, coin = false) {
    ...
    return { txId: tx.id, outputIndex: vout, script: output.script.toHex(), satoshis: output.satoshis,
        ...(coin ? { ftBalance: coinTbc20Code_1.CoinTBC20.parseTape(tx.outputs[vout + 1].script).balance } : {}) };
}
...
class Coin extends StableCoin {
    ...
    initialize(info) {
        ...
        const code = coinTbc20Code_1.CoinTBC20.parseCode(info.codeScript);
        coinTbc20Code_1.CoinTBC20.parseTape(info.tapeScript, code);
        ...
        this.codeScript = info.codeScript;
        this.tapeScript = info.tapeScript;
    ...
    buildIssuanceScripts(adminHash, recipient, issuerHash, raw) {
        ...
        let tapeScript;
        if (this.codeScript) {
            const previous = coinTbc20Code_1.CoinTBC20.parseCode(this.codeScript);
            ...
            tapeScript = coinTbc20Code_1.CoinTBC20.setLockTime(coinTbc20Code_1.CoinTBC20.replaceTapeAmounts(this.tapeScript, [raw, 0n, 0n, 0n, 0n, 0n]), 0);
        }
        else {
            ...
            tapeScript = coinTbc20Code_1.CoinTBC20.buildTape({ amounts: [raw, 0n, 0n, 0n, 0n, 0n], tapeSize: 66 + metadata.length, lockTime: 0, metadata });
        }
    ...
    mintCoin(admin, feeKey, recipient, humanAmount, fee, parent, grandparent, message) {
        const code = coinTbc20Code_1.CoinTBC20.parseCode(this.codeScript);
    ...
    static buildCoinNftTX(feeKey, adminHash, fee, data) {
        ...
        const outputs = Coin.buildCoinNftOutput(TBC721.buildCodeScript(fee.txId, fee.outputIndex), TBC721.getHoldScriptFromHash(adminHash, data.nftName), certificateTape(data));
    ...
    inputs(utxos, parents, proofs) {
        ...
        const identity = coinTbc20Code_1.CoinTBC20.getCodeIdentity(this.codeScript);
        const seen = new Set();
        return utxos.map((utxo, i) => {
            ...
            seen.add(point);
            const descriptor = coinTbc20Code_1.CoinTBC20.parseCode(utxo.script);
            ...
            const parsed = coinTbc20Code_1.CoinTBC20.parseTape(tape, descriptor);
    ...
    build(inputs, fee, feeKey, allocations, signer, extra) {
        ...
        for (const allocation of allocations) {
            ...
            tx.addOutput(new tbc.Transaction.Output({ satoshis: 500, script: coinTbc20Code_1.CoinTBC20.replaceController(this.codeScript, allocation.controller) }));
            tx.addOutput(new tbc.Transaction.Output({ satoshis: 0, script: coinTbc20Code_1.CoinTBC20.setLockTime(coinTbc20Code_1.CoinTBC20.replaceTapeAmounts(allocation.tape, allocation.amounts), allocation.lockTime) }));
    ...
    mergeCoin(key, utxos, fee, parents, proofs, localTX = []) {
        ...
        while (pending.length > 1) {
            ...
            pending.unshift({ utxo: coin, parent: tx, ancestors: local, descriptor: coinTbc20Code_1.CoinTBC20.parseCode(coin.script),
    ...
    static getCoinMintCode(adminHash, recipient, issuerHash, tapeSize) {
        ...
        return coinTbc20Code_1.CoinTBC20.instantiateCode({ adminPubKeyHash: Buffer.from(adminHash, 'hex'), coinNftCodeHash: Buffer.from(issuerHash, 'hex'), tapeSize, controller: controller(recipient) });
    }
    static setLockTimeInTape(tape, lockTime) {
        return coinTbc20Code_1.CoinTBC20.setLockTime(tape, lockTime);
    }
    static getLockTimeFromTape(tape) { return coinTbc20Code_1.CoinTBC20.parseTape(tape).lockTime; }
    static getAddressFromCode(code) {
        const data = coinTbc20Code_1.CoinTBC20.parseCode(code).controller;
        return { address: data.subarray(0, 20).toString('hex'), type: data[20] === 0 ? 'address' : 'contract' };
    }
    static buildFTtransferCode(code, address) {
        return coinTbc20Code_1.CoinTBC20.replaceController(code, controller(address));
    }
    static buildFTtransferTape(tape, amountHex) {
        ...
        return coinTbc20Code_1.CoinTBC20.replaceTapeAmounts(tape, Array.from({ length: 6 }, (_, i) => bytes.readBigUInt64LE(i * 8)));
    }
    static buildUTXO(tx, vout) {
        const output = tx.outputs[vout];
        if (!output)
            fail('Coin output index is out of range');
        const code = coinTbc20Code_1.CoinTBC20.parseCode(output.script);
        coinTbc20Code_1.CoinTBC20.parseTape(tx.outputs[vout + 1]?.script, code);
    ...
    _mergeCoin(...args) { fail('use mergeCoin for Coin TBC20'); }
    _mergeFT(...args) { fail('use mergeCoin for Coin TBC20'); }
    mergeFT_(...args) { fail('use mergeCoin for Coin TBC20'); }
    _batchTransfer(...args) { fail('use batchTransfer for Coin TBC20'); }
    _batchTransfer_old(...args) { fail('use batchTransfer for Coin TBC20'); }
    static getBalanceFromTape(tape) {
        return coinTbc20Code_1.CoinTBC20.parseTape(tape).balance;
    }
    MintFT(...args) { fail('Coin issuance requires createCoin or mintCoin'); }
    getFTmintCode(...args) { fail('Coin issuance requires getCoinMintCode'); }
    transferContract(...args) { fail('use getUnlockScript with an explicit contractController witness for contract-held Coin'); }
    getFTunlock(...args) { fail('use Coin.getUnlockScript for the Coin TBC20 ABI'); }
    getFTunlockSwap(...args) { fail('use Coin.getUnlockScript with contractController'); }
    static getFTunlock(...args) { fail('use Coin.getUnlockScriptWithSignature for Coin TBC20'); }
    static getFTunlockSwap(...args) { fail('use Coin.getUnlockScriptWithSignature with contractController'); }
}
module.exports = Coin;
