...
function value(output: tbc.Transaction.Output): Buffer {
  if (!Number.isSafeInteger(output.satoshis) || output.satoshis < 0) fail('invalid output satoshis');
  return new tbc.encoding.BufferWriter().writeUInt64LEBN(output.satoshisBN).toBuffer();
}

function transaction(tx: tbc.Transaction, label: string): void {
  if (!(tx instanceof tbc.Transaction) || (tx as any).version !== 10) fail(`${label} must be a version 10 transaction`);
  if (!tx.inputs.length || !tx.outputs.length) fail(`${label} must contain inputs and outputs`);
}

function header(tx: tbc.Transaction): Buffer {
  return Buffer.concat([u32((tx as any).version), u32(tx.nLockTime), u32(tx.inputs.length), u32(tx.outputs.length)]);
}

function inputs(tx: tbc.Transaction): Buffer {
...
function unlockingHash(tx: tbc.Transaction): Buffer {
  return sha(Buffer.concat(tx.inputs.map(input => sha(input.script.toBuffer()))));
}

function records(outputs: tbc.Transaction.Output[]): Buffer {
...
function push(script: tbc.Script, data: Buffer): void {
  if (data.length === 1 && data[0] >= 1 && data[0] <= 16) script.add(0x50 + data[0]);
  else if (data.length === 1 && data[0] === 0x81) script.add(tbc.Opcode.OP_1NEGATE);
...
function linked(input: tbc.Transaction.Input, parent: tbc.Transaction, index: number, label: string): void {
...
export function buildTBC721UnlockScript(
  ...
  if (currentUnlockIndex !== 0) fail('Code must be spent at input 0');
  ...
  if (!Buffer.isBuffer(publicKey) || ![32, 33, 65].includes(publicKey.length)) fail('invalid public key length');
  if (preTX.outputs.length < 2) fail('parent must contain Code and Hold outputs');
  ...
  if (!tbc.crypto.Hash.sha256ripemd160(publicKey).equals(hold.subarray(3, 23))) fail('public key does not match the parent Hold owner');
  ...
  for (const match of (artifact.unlock.main as string).matchAll(/<([^>]+)>/g)) {
    const data = match[1].split('.').reduce((cursor, field) => cursor?.[field], args as any);
    if (!Buffer.isBuffer(data)) fail(`missing ABI field ${match[1]}`);
    push(result, data);
  }
  return result;
}
