...
const ftscript_1 = require("./ftscript");
...
const SERVICE_FEE_PKH_SET = new Set(Object.values(SERVICE_FEE_ADDRESS).map(addr => tbc.Address.fromString(addr).hashBuffer.toString('hex')));
function isServiceFeePkh(pubKeyHash) {
...
const getFTPartialOffset = (codeLength) => {
    return (0, ftscript_1.getFTPartialOffsetByLength)(codeLength) ?? ftscript_1.FT_V2_PARTIAL_OFFSET;
};
function getInputsTxdata(tx, vout) {
    ...
    if ((0, ftscript_1.isFTCodeLength)(lockingscript.length)) {
...
function getCurrentTxOutputsDataforPool2(tx, option, withLock, swapOption) {
    const writer = new tbc.encoding.BufferWriter();
    let lockingscript = tx.outputs[2].script.toBuffer(); //FTAbyC code部分
                ...
                case 1:
                    ...
                    if (tx.outputs.length == 5) {
                        ...
                        if (!isServiceFeePkh(pubKeyHash)) {
                            ...
                            writer.write(Buffer.from(amountlength, 'hex'));
                            writer.writeUInt64LEBN(tx.outputs[6].satoshisBN);
                            writer.write(getLengthHex(tx.outputs[6].script.toBuffer().length));
                            writer.write(tx.outputs[6].script.toBuffer());
                            writer.write(Buffer.from('00', 'hex'));
                        }
                    }
                    ...
                    switch (tx.outputs.length) {
                        ...
                        case 8:
                            ...
                            if (!isServiceFeePkh(pubKeyHash)) {
                                ...
                                writer.write(Buffer.from(amountlength, 'hex'));
                                writer.writeUInt64LEBN(tx.outputs[7].satoshisBN);
                                writer.write(getLengthHex(tx.outputs[7].script.toBuffer().length));
                                writer.write(tx.outputs[7].script.toBuffer());
                                writer.write(Buffer.from('00', 'hex'));
                            }
                            break;
                        //完整输出
