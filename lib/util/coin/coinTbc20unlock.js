...
const coinTbc20Code_1 = require("./coinTbc20Code");
...
function ancestors(options, descriptor, amounts) {
    ...
    for (let slot = 0; slot < 6; slot += 1) {
        ...
        try {
            sameIdentity = coinTbc20Code_1.CoinTBC20.getCodeIdentity(code).equals(descriptor.identity);
        }
        catch {
...
function verifyOutputAllocations(options, descriptor, inputBalance) {
    ...
    for (const group of options.outputGroups) {
        ...
        if (amount === 0n)
            continue;
        const recipient = coinTbc20Code_1.CoinTBC20.validateCode(code.script, {
        ...
        coinTbc20Code_1.CoinTBC20.parseTape(tape.script, recipient);
...
function buildCoinTBC20UnlockScriptWithSignature(options) {
    ...
    const descriptor = coinTbc20Code_1.CoinTBC20.parseCode(options.preTx.outputs[options.preTxVout].script);
    const preData = (0, tbc20unlock_1.getTBC20PreTxData)(options.preTx, options.preTxVout);
    const tape = coinTbc20Code_1.CoinTBC20.parseTape(preData.outputsGotData.tape.lockingScript, descriptor);
    ...
    coinTbc20Code_1.CoinTBC20.verifyInputLock(options.currentTx, options.inputIndex, preData.outputsGotData.tape.lockingScript, descriptor, administrator);
