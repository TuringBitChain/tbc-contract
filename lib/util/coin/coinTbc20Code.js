...
exports.CoinTBC20 = void 0;
const node_crypto_1 = require("node:crypto");
const tbc = __importStar(require("tbc-lib-js"));
const tbc20unlock_1 = require("../tbc20/tbc20unlock");
const ARTIFACT = require('./artifacts/coin_tbc20.json');
if ((0, node_crypto_1.createHash)('sha256').update(JSON.stringify(ARTIFACT)).digest('hex') !==
...
function fail(message) { throw new Error(`Coin TBC20: ${message}`); }
...
function assertBytes(value, width, name) {
    if (!Buffer.isBuffer(value) || value.length !== width)
        fail(`${name} must be ${width} bytes`);
}
function assertTapeSize(size) {
    if (!Number.isInteger(size) || size < 66 || size > 127)
        fail('tapeSize must be 66-127 bytes');
}
function assertLockTime(value) {
    if (!Number.isInteger(value) || value < 0 || value > UINT32_MAX)
        fail('lockTime must be an unsigned 32-bit integer');
}
function assertAmounts(amounts) {
    if (!Array.isArray(amounts) || amounts.length !== 6)
...
function assertMetadata(metadata) {
    if (!Buffer.isBuffer(metadata))
        fail('metadata must be a Buffer containing a complete push-only script');
    try {
        const script = tbc.Script.fromBuffer(metadata);
        if (!script.isPushOnly() || !script.toBuffer().equals(metadata))
...
class CoinTBC20 {
    ...
    static codeSize = 2981;
    static partialOffset = 2944;
    ...
    static instantiateCode(options) {
        ...
        if (hex.includes('<'))
            fail('unresolved Coin constructor parameter');
        const script = tbc.Script.fromHex(hex);
        if (script.toBuffer().length !== CoinTBC20.codeSize || (0, tbc20unlock_1.getTBC20PartialScriptData)(script).suffixData.length !== 37)
    ...
    static parseCode(value) {
        ...
        for (const segment of template().split(PLACEHOLDER)) {
            if (segment.startsWith('<self.')) {
                ...
                if (offset + width + 1 > code.length || code[offset] !== width)
                    fail('unsupported or modified Coin Code template');
                const data = code.subarray(offset + 1, offset + width + 1);
                if (fields.has(segment) && !fields.get(segment).equals(data))
                    fail('inconsistent Coin constructor parameter');
        ...
        if (offset !== code.length)
            fail('unsupported or modified Coin Code length');
        const options = {
        ...
        const partial = (0, tbc20unlock_1.getTBC20PartialScriptData)(CoinTBC20.instantiateCode(options));
    ...
    static validateCode(value, expected = {}) {
        ...
        for (const field of ['coinNftCodeHash', 'adminPubKeyHash', 'controller']) {
    ...
    static getCodeIdentity(value) { return CoinTBC20.parseCode(value).identity; }
    static replaceController(value, controller) {
        ...
        if (!CoinTBC20.getCodeIdentity(script).equals(previous.identity))
            fail('controller replacement changed Coin identity');
        return script;
    }
    static buildTape(options) {
        ...
        if (metadata.length > options.tapeSize - 66)
            fail('metadata exceeds the fixed Coin Tape capacity');
    ...
    static parseTape(value, profile = {}) {
        ...
        if (profile.tapeSize !== undefined && profile.tapeSize !== bytes.length)
            fail('Coin Tape length does not match Code');
    ...
    static replaceTapeAmounts(value, amounts) {
        return CoinTBC20.buildTape({ ...CoinTBC20.parseTape(value), amounts });
    }
    static setLockTime(value, lockTime) {
    ...
    static getRequiredLockTime(lockTimes) {
        ...
        for (const lockTime of lockTimes) {
            ...
            if (lockTime !== 0) {
                ...
                if (domain !== undefined && domain !== timestamp)
                    fail('cannot combine nonzero height and timestamp Coin locks');
                domain = timestamp;
            }
    ...
    static verifyInputLock(tx, inputIndex, tape, profile = {}, administrator = false) {
        ...
        if (typeof administrator !== 'boolean')
            fail('administrator must be a boolean');
        ...
        if (tx.inputs[inputIndex].sequenceNumber === UINT32_MAX)
            fail('every Coin input needs a nonfinal sequence, including administrator and zero locks');
        if (administrator)
            return;
        ...
        if (parsed.lockTime > tx.nLockTime)
            fail('transaction nLockTime does not satisfy the parent Coin lock');
    }
}
exports.CoinTBC20 = CoinTBC20;
exports.default = CoinTBC20;
