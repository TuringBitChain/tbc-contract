...
function assertIndex(value: number, upperExclusive: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value >= upperExclusive) {
    fail(`${name} must be an integer in [0, ${upperExclusive - 1}]`);
  }
}

function assertVersion10Transaction(tx: tbc.Transaction, name: string): void {
  assertTransaction(tx, name);
  const version = (tx as any).version;
  if (version !== 10) {
...
function toBuffer(value: string | Buffer, name: string): Buffer {
...
function toPublicKeyBuffer(value: string | Buffer | tbc.PublicKey): Buffer {
  let buffer: Buffer;
  ...
  try {
    ...
    fail(`publicKey is invalid: ${(error as Error).message}`);
  }
  return buffer;
}

function toSignatureBuffer(value: string | Buffer): Buffer {
  ...
  try {
    ...
    fail(`signature cannot be decoded: ${(error as Error).message}`);
  }
...
function txVLIO(tx: tbc.Transaction): Buffer {
  const writer = new tbc.encoding.BufferWriter();
  writer.writeUInt32LE((tx as any).version);
...
function txInputRecords(tx: tbc.Transaction): Buffer {
  return Buffer.concat(tx.inputs.map((input: any) => inputRecord(input)));
}

function txUnlockingScriptsHash(tx: tbc.Transaction): Buffer {
  const hashes = tx.inputs.map((input: any) => {
...
function txInputsHashData(tx: tbc.Transaction): Buffer {
...
function outputHashRecord(output: tbc.Transaction.Output, name: string): Buffer {
...
export function getTBC20PartialScriptData(script: tbc.Script): TBC20PartialScriptData {
  ...
  const partialHashHex = partialSha256.calculate_partial_hash(
    lockingScript.subarray(0, partialOffset),
  );
...
export function getTBC20CurrentOutputData(
  ...
  outputGroups.forEach((group, groupIndex) => {
    ...
    let tapeOutput: tbc.Transaction.Output | undefined;
    ...
    groups.push(outputGroupData(
      tx.outputs[group.codeVout],
      tapeOutput,
      `currentTX.Outputs[${groupIndex}]`,
    ));
  });
...
export function getTBC20PreTxData(tx: tbc.Transaction, codeVout: number): TBC20PreTxData {
  ...
  return {
    ...
    outputsGotData: outputGroupData(
      tx.outputs[codeVout],
      tx.outputs[codeVout + 1],
      "preTX.OutputsGotData",
    ),
...
export function getTBC20PrePreTxArray(
  ...
  for (let parentInputIndex = 0; parentInputIndex < TBC20_MAX_INPUTS; parentInputIndex += 1) {
    ...
    const parentInput: any = preTx.inputs[parentInputIndex];
...
function assertCurrentInputLinksPreTx(
  currentTx: tbc.Transaction,
  inputIndex: number,
  preTx: tbc.Transaction,
  preTxVout: number,
  ...
  const input: any = currentTx.inputs[inputIndex];
...
function resolveContractWitness(
  ...
  const currentInput: any = currentTx.inputs[witness.currentInputIndex];
...
function addScriptNumber(script: tbc.Script, value: number, name: string): void {
  if (!Number.isSafeInteger(value) || value < 0) {
    fail(`${name} must be a non-negative safe integer`);
  }
  if (value <= 16) {
...
export function buildTBC20UnlockScriptWithSignature(
  ...
  addScriptNumber(
    unlockingScript,
    contract.currentInputIndex,
    "contractController.currentInputIndex",
  );
