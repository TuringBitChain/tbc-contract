...
function getPreTxdata(tx, vout, contractOutputNumber) {
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
        ...
        if (isCurrentContract) {
            ...
            else if (len === tokenOrderCodeLength)
                partialOffset = tokenOrderPartialOffset;
...
function getCurrentTxOutputsData(tx, fixedOutputCount = 10) {
    const writer = new tbc.encoding.BufferWriter();
    for (let i = 0; i < tx.outputs.length; i++) {
        const lockingscript = tx.outputs[i].script.toBuffer();
        const len = lockingscript.length;
        const size = getSize(len);
        let partialOffset = token_1.modernCodeOffsets.get(len) ?? 0;
        if (len === ftscript_1.FT_V2_CODE_LENGTH)
            partialOffset = ftscript_1.FT_V2_PARTIAL_OFFSET;
        else if (len === ftscript_1.LEGACY_COIN_CODE_LENGTH)
            partialOffset = ftscript_1.LEGACY_COIN_PARTIAL_OFFSET;
        else if (len === ftscript_1.FT_V4_CODE_LENGTH)
            partialOffset = ftscript_1.FT_V4_PARTIAL_OFFSET;
        ...
        writer.write(getLengthHex(size.length));
        writer.write(size);
        if (token_1.modernCodeOffsets.has(len) ||
            len === ftscript_1.FT_V2_CODE_LENGTH ||
            len === ftscript_1.LEGACY_COIN_CODE_LENGTH ||
            len === ftscript_1.FT_V4_CODE_LENGTH) {
    ...
    const paddingCount = missingOutputCount > 0 ? missingOutputCount * 4 - 2 : 0;
